...
class PlatformLabelStudio(LabelStudioMLBase):
    """
    基于 TorchScript 的单图多标签图像分类 ML Backend。
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.DEFAULT_MODEL_SCORE_THRESHOLD = float(kwargs.pop("model_score_threshold", 0.25))
        base_kwargs = {
            key: kwargs[key]
        ...
        logger.info(
            "初始化多标签图像分类后端实例：模型目录=%s，模型文件=%s，默认阈值=%s",
            self.MODEL_DIR,
            self.MODEL_PATH,
    ...
    def predict(self, tasks: List[Dict], context: Optional[Dict] = None, **kwargs) -> ModelResponse:
        ...
        try:
            ...
            logger.exception("获取多标签图像分类标签配置失败")
        ...
        for task in tasks:
            try:
                ...
                logger.info("开始处理多标签图像分类任务，任务编号=%s", task_id)
