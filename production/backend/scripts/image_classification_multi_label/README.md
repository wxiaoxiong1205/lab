...
这是一个基于 PyTorch 的多标签图片分类示例，重点在于实现 `model.py` 中的模型输入处理和输出处理逻辑。
...
- `labels`：所有通过阈值筛选的标签集合。
- `labels` 中的每个元素都包含 `class_id` 和 `score` 两个字段。

常见处理流程：

- 兼容不同输出格式，例如 `tensor`、`list`、`tuple` 或 `dict`。
- 提取分类 logits，并确认输出形状符合常见格式 `[B, C]`。
- 对 logits 做 `sigmoid`，得到各类别的独立概率。
- 按 `threshold` 筛选所有满足条件的类别，而不是只保留 top1。
- 将命中的类别整理为标签列表返回，每个标签都包含 `class_id` 和 `score`。
