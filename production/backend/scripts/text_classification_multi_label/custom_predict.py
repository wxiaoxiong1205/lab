...
class PlatformLabelStudio(LabelStudioMLBase):
    """
    基于 TorchScript 的多标签文本分类 ML Backend。
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.DEFAULT_MODEL_SCORE_THRESHOLD = float(kwargs.pop("model_score_threshold", 0.4))
        ...
        logger.info(
            "初始化多标签文本分类后端实例：模型目录=%s，模型文件=%s，默认阈值=%s",
            self.MODEL_DIR,
            self.MODEL_PATH,
            self.DEFAULT_MODEL_SCORE_THRESHOLD,
        )
    ...
    def _assert_multiple_choice_mode(self, from_name: str):
        ...
        if choice_mode != "multiple":
            raise AssertionError(
                f"当前 demo 仅支持 `Choices choice=\"multiple\"`，实际为 choice={choice_mode!r}"
            )
    ...
    def _get_model_score_threshold(self, from_name: str) -> float:
        ...
        raw_value = (
            ...
            or getattr(control, "model_score_threshold", None)
            or self.DEFAULT_MODEL_SCORE_THRESHOLD
        )
        ...
        try:
            ...
            logger.warning(
                "控件 `%s` 的 model_score_threshold=%r 不是合法浮点数，已回退到默认值 %.2f",
                from_name,
                raw_value,
                self.DEFAULT_MODEL_SCORE_THRESHOLD,
            )
            return self.DEFAULT_MODEL_SCORE_THRESHOLD
    ...
    def _validate_multi_label_model_output(
        self,
        model_output,
    ...
    def _build_prediction_region(
        ...
        if not prediction_result.labels:
            return None
        ...
        for item in prediction_result.labels:
            predicted_class_id = item.class_id + 1
            predicted_label = class_label_mapping.get(predicted_class_id)

            if predicted_label is None:
                logger.warning(
                    "预测类别 id=%s（模型原始 class_id=%s）未在 label_config 中找到对应标签，当前映射键=%s，已跳过该标签",
                    predicted_class_id,
                    item.class_id,
            ...
            if predicted_label in seen_labels:
                continue

            seen_labels.add(predicted_label)
        ...
        region_score = sum(scores) / len(scores)
        return {
            "id": str(uuid4())[:8],
            "from_name": from_name,
            "to_name": to_name,
    ...
    def predict(self, tasks: List[Dict], context: Optional[Dict] = None, **kwargs) -> ModelResponse:
        ...
        try:
            ...
            self._assert_multiple_choice_mode(from_name)
            ...
            logger.exception("获取多标签文本分类标签配置失败")
        ...
        for task in tasks:
            try:
                ...
                logger.info("开始处理多标签文本分类任务，任务编号=%s", task_id)
                ...
                self._validate_multi_label_model_output(outputs, class_label_mapping)
                prediction_result = self.model_handle.post_handle(
                    outputs,
                ...
                result_score = float(region["score"]) if region else 0.0
                ...
                logger.info(
                    "任务编号=%s 预测完成：命中标签数=%s，阈值=%.4f，score=%.6f",
                    task_id,
                    len(prediction_result.labels),
