...
def get_common_config_data() -> List[Dict[str, Any]]:
    """获取通用配置种子数据"""
    return [
        {
            "config_key": "prompt_template",
            "config_value": """你是一个严谨专业的文本评估器，负责依据给定的评估标准对文本进行评分，并给出合理的评分理由。
...
{% if expected_output is defined %}

<expected_output>期望回答:

{{ expected_output_mapping_value }}

</expected_output>

{% endif %}

{% if retrieval_context is defined %}

<retrieval_context>检索上下文:

{{ retrieval_context_mapping_value }}
...
1. 请根据「评估指标定义」和「待评估内容」进行评分。

2. 打分时，只考虑与当前指标「{{ metric.name }}」相关的因素。

3. 给出的评分理由需为简明但具体的中文说明。



[输出格式]

只输出下面的 JSON 结构，不要包含任何额外文字或解释：
