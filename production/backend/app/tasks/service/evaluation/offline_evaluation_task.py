...
from app.schemas.evaluation_task import InferenceParamType, EvaluationMethod
...
class OfflineEvaluationTaskK8s(BaseK8sTask):
    """离线评估任务K8s封装类"""
    ...
    async def build_volume_and_mount(self) -> Tuple[List[V1VolumeMount], List[V1Volume]]:
        ...
        # 创建进度文件和统计文件（评估任务需要根据 evaluation_method 创建不同的进度文件）
        ...
        # 裁判员评估时根据 referee_model_source 选择模型存储
        referee_model_source = (self.evaluation_task.referee_model_source or "base_model").lower()
        if self.evaluation_task.referee_type == "model" and referee_model_source == "trained_model":
            referee_model_storage = {"name": PvcName.LLM_TRAINING_PVC.value, "enum": StoragePath.INFEERENCE_TRAINED_MODELS}
        else:
            referee_model_storage = {"name": "public-pvc", "enum": StoragePath.BASE_MODELS}
    ...
    async def build_cmd_and_args(self) -> List[Tuple[List[str], List[str]]]:
        ...
        def build_basic_metric_cmd_args():
            ...
            for metric_name in metrics_list:
                ...
                if metric_name in ["accuracy", "f1", "rouge-1", "rouge-2", "rouge-l", "bleu-4", "format_compliance", "semantic_similarity"]:
        ...
        def build_referee_cmd_args():
            # 构建进度文件路径（添加前缀以区分裁判员评估）
            # 注意：裁判员评估的进度文件在 evaluate_script 中通过 config 文件配置
            # 这里需要在 config 文件中设置，但为了保持一致性，我们也在这里处理
            command = ["python3"]
    ...
    async def build_image(self) -> str:
        ...
        if self.evaluation_task.graphics_card_resource:
            graphics_card_resource = GraphicsCardResourceConfig(**self.evaluation_task.graphics_card_resource)
            # 获取显卡类型和型号
            card_category = graphics_card_resource.card_type.value \
                if (hasattr(graphics_card_resource.card_type, 'value')) \
                else str(
                graphics_card_resource.card_type)
            card_model = graphics_card_resource.card_model.value \
                if hasattr(graphics_card_resource.card_model, 'value') \
                else str(
                graphics_card_resource.card_model) if graphics_card_resource.card_model else None
            ...
            # 如果没有配置，使用默认值
            # 在线评估使用 CPU 镜像
            logger.info(f"没有配置资源相关的配置，直接走cpu的资源，兼容基础指标评估")
            image_address = await self.find_image_address_by_project(
                project_id=self.project_id,
                image_type=ImageType.INFERENCE_IMAGE.value,
        ...
        if not image_address:
            raise RuntimeError(
                f"未找到匹配的离线评估镜像: project_id={self.project_id}, graphics_card_resource={graphics_card_resource}"
            )
    ...
    async def build_run_resource(self) -> Dict[str, Any]:
        """
        构建运行资源（CPU、内存、GPU等），这里仅对裁判员评估负责，基础指标评估在submit的逻辑中，清空掉
    ...
    async def build_job_name(self) -> str:
        ...
        Returns:
            str: Job名称
        """
        return f"offline-evaluation-{self.task_id}"
    ...
    async def submit(self) -> str:
        ...
        # 打印关键配置信息
        logger.info(f"离线评估任务配置 - task_id: {self.task_id}, 评估方法: {self.evaluation_task.evaluation_method}")
        logger.info(f"使用镜像: {image}")
        logger.info(f"仅对裁判员评估生效 GPU配置: type={resources['gpu_type']}, count={resources['gpu_count']}")
        ...
        if evaluation_method == "all":
            ...
            basic_metric_image_address = await self.find_image_address_by_project(
                project_id=self.project_id,
                image_type=ImageType.INFERENCE_IMAGE.value,
                card_category=CardType.CPU.value
            )
            ...
            containers_config.append({
                ...
                "image": basic_metric_image_address,
            ...
            # 这里增加基础指标评估的额外判断以修复基础指标评估占用gpu的问题, 这里为单个的情况
            if evaluation_method == EvaluationMethod.BASIC_METRIC.value:
    ...
    async def build_prompt_config(self):
        ...
        if not prompt_template:
            ...
            try:
                # 获取项目根目录 (app/tasks/service/evaluation/offline_evaluation_task.py -> 5 levels up)
                ...
                logger.error(f"加载默认 prompt 模板失败: {e}")
                return
    ...
    async def build_config(self):
        ...
        为裁判员评估任务构建推理配置文件，包含推理参数和vLLM客户端配置
        """
        if self.evaluation_task.evaluation_method not in ["referee", "all"]:
            # 只有裁判员评估需要推理配置
            return
        ...
        try:
            ...
            # 获取裁判模型信息（支持 base_model 和 trained_model）
            referee_model_source = (self.evaluation_task.referee_model_source or "base_model").lower()
            if referee_model_source == "trained_model":
                trained_model: TrainedModel = await self.db.query_one(
                    select(TrainedModel).filter(TrainedModel.id == self.evaluation_task.referee_model_id)
                )
                if not trained_model or not trained_model.model_path:
                    raise RuntimeError(f"未找到训练裁判模型或模型路径为空: {self.evaluation_task.referee_model_id}")
                ...
                referee_model_name = trained_model.name
            else:
                base_model: BaseModel = await self.db.query_one(
                    select(BaseModel).filter(BaseModel.id == self.evaluation_task.referee_model_id)
                )
                if not base_model:
                    raise RuntimeError(f"未找到裁判模型: {self.evaluation_task.referee_model_id}")
                model_path = StoragePath.BASE_MODELS.mount_path + base_model.model_path.replace("/public/models/", "")
                referee_model_name = base_model.name
            ...
            config = {
                ...
                "runtime": {
                    ...
                    "inference_result_dataset_id":
                        list(self._inference_dataset_info.keys()) if self._inference_dataset_info else []
                },
                "data": {
            ...
            if self.evaluation_task.referee_inference_params:
                # 复制一份参数，避免修改原始数据
                inf_params = self.evaluation_task.referee_inference_params.copy()
    ...
    async def _build_metrics_config(self) -> Optional[List[Dict[str, Any]]]:
        ...
        for metric in metrics:
            ...
            # 处理分值范围
            # 如果 metric 中有 score_min 和 score_max，直接使用

            # 如果没有，尝试从 system_metric_id 获取
            system_metric_id = metric.get("system_metric_id")
            if system_metric_id:
                try:
                    logger.info(f"taskid = {self.task_id},指标id = {system_metric_id}")
                    ...
                    evaluation_metric = await self.db.query_one(
                    ...
                    if evaluation_metric and evaluation_metric.score_scope and len(evaluation_metric.score_scope) > 0:
                        logger.info(f"有效裁判员指标，metric = {evaluation_metric}")
                        ...
                        logger.error(f"系统指标 {system_metric_id} 没有分值范围配置")
                except Exception as e:
                    logger.error(f"查询系统指标 {system_metric_id} 失败: {e}")
            else:
                logger.error(f"指标 {metric.get('name')} 缺少指标配置")
