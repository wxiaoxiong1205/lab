...
用于将基准评估任务的基本信息和评估结果导出为 DOCX 格式文件，包含文本内容和图表。
结构参考 evaluation_report_docx_generator。
"""
...
# 配置 matplotlib 中文字体：优先使用系统实际存在的中文字体文件，避免 Linux 等环境仅剩 DejaVu Sans 导致图表中文乱码
...
class BenchmarkReportDocxGenerator:
    """基准评估报告 DOCX 生成器"""

    def __init__(
        self,
        task_data: BenchmarkTaskDetailResponse,
        report_data: BenchmarkTaskReportResponse,
    ):
        """
        Args:
            task_data: 基准评估任务基本信息（来自 get_task）
            report_data: 评估报告数据（来自 get_task_report）
        """
    ...
    def generate(self) -> bytes:
        ...
        try:
            ...
            logger.error(f"生成基准评估DOCX失败: {str(e)}", exc_info=True)
            raise

    def _add_title(self):
        """添加报告标题"""
        title = self.doc.add_heading('基准评估报告', 0)
    ...
    def _add_basic_info(self):
        """添加基本信息章节（任务名称、创建人、待评估模型/服务、创建时间、基准评估数据集、描述）"""
        self.doc.add_heading('一、任务基本信息', 1)
        evaluated_models = [m.model_name for m in (self.task_data.models or [])]
        datasets = [d.dataset_name for d in (self.task_data.datasets or [])]
        created_at = getattr(self.task_data, 'created_at', None) or self.task_data.started_at
        created_by = getattr(self.task_data, 'created_by', None) or '无'
        created_at_str = created_at.strftime("%Y-%m-%d %H:%M:%S") if created_at else '无'
        basic_info_fields = [
            ('任务名称', self.task_data.name),
            ('创建人', created_by),
            ('待评估模型/服务', '，'.join(evaluated_models) if evaluated_models else '无'),
            ('创建时间', created_at_str),
            ('基准评估数据集', '，'.join(datasets) if datasets else '无'),
            ('描述', self.task_data.description or '无'),
        ]
    ...
    def _get_dataset_codes_ordered(self) -> List[str]:
        """获取评估指标（数据集 code）有序列表，按接口返回顺序（与 benchmark_compare 一致）"""
        if self.task_data.datasets:
            return [d.dataset_code for d in self.task_data.datasets]
        if not self.report_data.model_reports:
            return []
        # 以第一份报告的 dataset_scores 键顺序为准，其余报告中有但未出现的 code 追加到末尾
        first = self.report_data.model_reports[0]
        ordered = list(first.dataset_scores.keys())
        seen = set(ordered)
        for r in self.report_data.model_reports[1:]:
            for c in r.dataset_scores.keys():
                if c not in seen:
                    ordered.append(c)
                    seen.add(c)
        return ordered
    ...
    def _add_model_results(self):
        ...
        if not self.report_data.model_reports:
            ...
            return
        dataset_codes = self._get_dataset_codes_ordered()
        if not dataset_codes:
            self.doc.add_paragraph("暂无评估指标数据。")
            return
        # 使用数据集名称作为雷达图/表标签（若有），否则用 code
        code_to_name = {d.dataset_code: d.dataset_name for d in (self.task_data.datasets or [])}
        metric_labels = [code_to_name.get(c, c) for c in dataset_codes]

        self.doc.add_heading('1. 基准评估', 2)
        ...
        try:
            ...
            logger.warning(f"生成基准评估雷达图失败: {str(e)}")
    ...
    def _add_detail_table(self, dataset_codes: List[str], metric_labels: List[str]):
        """评估数据明细表：评估指标 | 模型1 | 模型2 | …"""
        model_names = [r.model_name for r in self.report_data.model_reports]
        ...
        for idx, code in enumerate(dataset_codes):
            ...
            for i, model_report in enumerate(self.report_data.model_reports):
                score = model_report.dataset_scores.get(code)
    ...
    def _generate_radar_chart(self, dataset_codes: List[str], metric_labels: List[str]) -> Optional[bytes]:
        """雷达图：每个顶点为数据集指标，每条线代表一个模型，数值为原始得分（0-100 尺度）。"""
        if not dataset_codes or not self.report_data.model_reports:
            return None
        try:
            ...
            for idx, model_report in enumerate(self.report_data.model_reports):
                values = []
                for code in dataset_codes:
                    s = model_report.dataset_scores.get(code)
                    values.append((s or 0) / 100.0)
            ...
            logger.error(f"生成基准评估雷达图失败: {str(e)}", exc_info=True)
            plt.close()
            return None

    def _add_bar_charts(self):
        """三、评分对比柱状图：x 轴为评估指标（数据集），y 轴为分数，不同颜色柱体代表不同模型"""
        ...
        try:
            ...
            logger.warning(f"生成基准评估柱状图失败: {str(e)}")
    ...
    def _generate_comparison_bar_chart(self, dataset_codes: List[str], metric_labels: List[str]) -> Optional[bytes]:
        ...
        try:
            ...
            max_val = 0
            for i, model_report in enumerate(self.report_data.model_reports):
                scores = [model_report.dataset_scores.get(c) or 0 for c in dataset_codes]
            ...
            logger.error(f"生成基准评估柱状图失败: {str(e)}", exc_info=True)
            plt.close()
            return None
