...
- 方法路径：`POST /openapi/lab/v1/machine-learning-datasets/dataset/{project_id}/upload`
- Operation ID：`openapi_v1_machine_learning_datasets_create_machine_learning_dataset_with_file`
- 简述：上传文件创建机器学习数据集版本。

## 接口说明

- 请求体为 `multipart/form-data`。
- 上传数据集：`inherit_from_version=false` 时，通过 `chunk_upload_ids` 指定已完成的上传文件，创建数据集版本。
- 如需上传新版本，请使用“上传机器学习数据集新版本”接口。
