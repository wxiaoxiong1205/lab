...
interface RegistryClusterBindingModalProps {
  open: boolean
  onCancel: () => void
 ...
 * Harbor镜像仓库集群绑定管理组件
 */
const RegistryClusterBindingModal: React.FC<RegistryClusterBindingModalProps> = ({
  open,
  onCancel,
  registryId,
  registryName,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 调用API绑定集群
      await registryService.bindClusters(registryId, clusterIds)
  ...
  // 当Modal打开时加载数据
  useEffect(() => {
    if (open && registryId && registryName) {
      loadAvailableClusters()
    }
  }, [open, registryId, registryName])
  ...
  return (
    ...
    >
      <div className="py-4">
        <Alert
          message="集群绑定说明"
          description="选择要绑定到此镜像仓库的Kubernetes集群。绑定后，集群可以从该仓库拉取镜像。离线或错误状态的集群不可选择。"
          type="info"
          showIcon
          className="mb-4"
        />
        ...
        {loading ? (
          ...
          <>
            <div
              className="registry-transfer-container mb-4"
            >
              <Transfer
...
export default RegistryClusterBindingModal
