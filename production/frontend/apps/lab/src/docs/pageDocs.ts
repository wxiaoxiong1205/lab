...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', '模型仓库页面已按用户提供的全局任务流转规则收口。', '旧结构：模型提供商收敛为 Qwen，列表、表单和详情包含模型类型与支持能力字段；新结构：模型类型和支持能力均从列表、表单、详情和数据结构中删除，模型提供商放开为国内外主流开源提供商并支持自定义。', '新增模型弹窗取消本地上传来源，只保留 ModelScope 下载方式；本地模型改为通过 CLI 上传到模型仓库后在列表中查看。', '筛选区支持换行，列表设置明确列宽与横向滚动，避免小屏内容错位或右侧操作不可见。'],
      productionComparison: ['本轮未参考生产环境，按用户新增需求在当前仓库基线上迭代。'],
      userChanges: ['根据本轮需求，模型仓库取消页面本地上传方式，后续通过 CLI 上传模型至模型仓库；ModelScope 下载方式保持不变。', '根据本轮批注重做新增基础模型弹窗字段顺序和来源差异，并修复模型仓库小屏错位与无法横向滑动问题。', '根据最新批注将模型类型从 LLM/VLM 改为文本生成/图像理解，并将模型提供商收敛为 Qwen。', '根据本轮需求，模型仓库放开模型提供商限制，下拉支持国内外主流开源提供商和自定义，同时删除支持能力字段。', '根据本轮需求，模型仓库继续删除模型类型字段，训练创建页改为根据模型 Code/名称推断训练类型。', '根据本轮需求，被推理、部署或训练任务引用的模型仓库条目暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。'],
      syncMissingDefaultRequirements: true,
      defaultRequirements: [
        {
          version: 'V1.14',
