...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      productionComparison: ['生产代码已覆盖训练任务主链路；本次在生产 schema 与 additional_params 扩展口上补齐 RFT-GRPO 枚举和 GRPO 参数保存。', '生产替换期已确认：GRPO 参数模板不能直接使用 Demo localStorage 模板中心；当前已补生产后端持久化与系统设置 CRUD，后续继续接训练创建页模板选择和版本详情 YAML 快照展示。'],
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            acceptance: [
              ...
              '后续训练创建页接入后，新增或编辑配置可在大模型训练创建页 RFT-GRPO 场景选择。',
              '后续训练创建页接入后，停用配置不出现在创建页可选列表。',
              'YAML 根字段只允许 `fineTuneType` 和 `params`；输入奖励规则、DeepSpeed、模型、数据或资源字段时需要校验失败。',
            ],
            interactions: [
              '当前生产适配先保留 YAML 编辑；表单预览、训练创建页模板选择和版本详情 YAML 展示作为后续 GRPO 缺口继续迁移。',
            ],
          }),
        },
      ],
