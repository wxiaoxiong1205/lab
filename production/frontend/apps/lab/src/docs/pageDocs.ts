...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '版本状态', location: '详情页版本侧栏/版本表/基本信息', type: '状态标签', required: '是', description: '优先展示处理状态；处理成功后才展示发布状态，处理失败时版本侧栏显示处理失败，基本信息中的发布状态显示 -。' },
      ...
      actions: [
        ...
        { name: '合并版本', entry: '详情页左侧版本区合并版本按钮', precondition: '同一数据集内至少存在 2 个处理完成版本', successFeedback: '生成新的最新未发布版本，并在后端保留来源关系', errorFeedback: '版本不足或接口异常时提示' },
        { name: '发布当前版本', entry: '详情页顶部按钮', precondition: '当前版本处理完成且未发布', successFeedback: '当前版本发布后可被训练、标注和清洗引用', errorFeedback: '权限不足或接口异常时提示' },
        { name: '删除数据集/版本', entry: '列表删除或详情页版本删除', precondition: '当前数据集未被生产任务引用', successFeedback: '删除成功并刷新列表或详情', errorFeedback: '被标注、清洗、训练、推理或评估任务引用时提示阻断原因' },
      ...
      states: [
        { name: '处理中', meaning: '版本正在处理，还不能进入发布可用态', presentation: '详情页版本状态为处理中，发布状态字段为 -', availableActions: '查看基础信息' },
        { name: '处理成功', meaning: '版本处理完成，开始进入发布状态判断', presentation: '详情页状态字段展示处理成功，版本侧栏继续展示未发布或已发布', availableActions: '未发布时可发布，已发布时可用于下游' },
        ...
        { name: '处理失败', meaning: '版本处理失败，不能进入发布可用态', presentation: '详情页版本状态为处理失败，发布状态字段为 -', availableActions: '查看失败版本信息' },
      ],
      interactionNotes: ['页面头部不再展示面包屑，统一只保留标题、返回按钮和必要操作。', '去掉面包屑后，创建页、详情页和新增版本页顶部统一改为“返回 + 标题 + 说明/操作”的紧凑结构，避免出现大面积留白。', '详情页顶部标题只展示数据集名称，不提供编辑入口；名称编辑统一放在基本信息的数据集名称字段。', '列表筛选优先服务数据查找效率。', '列表筛选项默认提示词使用“数据用途”，返回列表时不保留旧筛选值。', '列表表格统一使用普通横向滚动，不再使用固定操作列，避免内容与操作区重叠。', '列表页只允许编辑数据集名称，不新增描述列；描述属于版本维度，只在详情页当前版本基本信息中编辑。', '列表行代表整个数据集，不代表某个具体版本；因此列表操作不展示发布按钮，版本发布统一进入详情页后针对当前版本操作。', '字段进入编辑态后不展示叉和勾按钮，按 Enter 或点击空白失焦即保存，Esc 取消本次编辑；详情页名称和描述字段后常显编辑按钮，编辑图标紧跟字段文本，不拉到单元格或字段容器右侧。', '左侧导航为固定侧栏时，主内容宽度需要扣除侧栏宽度，避免“100% 宽度 + 左边距”导致页面整体向右溢出。', '右侧需求文档侧板采用覆盖式抽屉，不再参与主内容宽度计算，避免窄屏或缩放时挤压列表并造成左右错位。', '详情页采用左侧版本切换、右侧信息与明细展示的结构，版本区按处理状态优先展示：处理中/处理失败直接显示处理状态，处理成功后显示未发布或已发布，并展示每个版本的创建人。', '详情页顶部需提供“返回列表”操作。', '数据删除必须有二次确认。', '删除数据集或版本前先用生产接口检查当前版本是否被标注或清洗任务占用；删除接口仍由生产后端按训练、标注、清洗、推理结果集和评估引用做最终阻断，并把 409 原因展示给用户。', 'DPO 数据详情按格式切换：Alpaca 展示 Instruction、Input、Chosen、Rejected；Role-Based 展示 Messages、Chosen、Rejected，其中 system 和 user 都在 messages 字段内。', '创建页的数据属性区域改为分组卡片结构，一个数据属性分组只保留一个可填值入口。', '创建页提交时如果存在未填写必填项，需要自动滚动并聚焦到首个错误字段，同时提示用户补充必填项。', '新增版本页保持生产环境基础布局，数据来源字段下移到继承历史版本下方，继承历史版本开启后仍允许继续点击上传文件追加内容，上传区不得置灰禁用。', '创建页和新增版本页需提供 JSONL / JSON / CSV 三种上传模板下载，安全盘点后移除前端 xlsx 依赖。', '涉及文件上传的区域统一展示上传进度；上传过程中允许取消并保留断点，失败或取消后可点击继续上传从当前进度恢复；文件大小不设前端限制。', '本页已基于用户新增需求补充 DPO / RFT-GRPO 对应数据用途，并改为单字段内的二级选择。', '为演示图像理解训练流程，训练数据列表补充 SFT、DPO、RFT-GRPO 多类图像理解训练数据集，', '根据本轮批注，图像理解 SFT 不再暴露 PROMPT_RESPONSE，列表与训练选择弹窗统一使用 ROLE_BASED。', '版本合并与新增版本放在同一版本操作区域；只要求源版本处理完成，不要求已发布，因此未发布但处理完成的版本也可以参与合并。', '合并生成的新版本默认为未发布，源版本保持不变，重复样本全部保留，并通过后端来源关系追踪数据来源；旧数据补齐的版本也必须参与真实持久化合并，成功后页面立即展示且刷新保留。', '版本侧栏操作区将新增版本和合并版本融合为同一组按钮，合并按钮在可合并版本不足 2 个时置灰。'],
      productionComparison: ['默认以生产环境训练数据管理为基线进行对齐。'],
      userChanges: ['如果后续增加字段或操作，需要同步更新页面文档侧板与 PRD/RPD 理解。', '根据用户新增需求，训练数据用途扩展为 SFT、DPO、RFT-GRPO 的文本生成/图像理解组合。', '根据本轮需求，DPO 数据结构从旧 CHOSEN_REJECTED 收敛为 Alpaca 与 Role-Based 两种格式；Role-Based 的 system/user 放在 messages 字段中，chosen/rejected 为 assistant 消息对象。', '根据本轮需求，训练数据详情页的“去训练”需要自动回填当前数据集到训练创建页。', '根据本轮批注，创建页删除 DPO 默认字段提示文案，数据属性改为分组卡片形式，详情页版本卡展示版本级状态与创建人。', '根据安全盘点结果，训练数据导出从 XLSX 改为 CSV，避免继续引入存在已知高危漏洞且无官方 npm 修复版本的 xlsx 依赖。', '根据本轮需求，被推理、标注、清洗或训练任务引用的数据集暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。', '根据本轮需求，数据集删除增加创建人权限校验，仅创建人本人、项目管理员或租户管理员可删除。', '根据本轮演示需求，新增多条图像理解训练数据集，覆盖 SFT、DPO、RFT-GRPO 场景。', '根据本轮批注，图像理解 SFT 数据格式去掉 PROMPT_RESPONSE，仅保留 ROLE_BASED。', '根据本轮批注，训练数据详情页每个版本补充创建人展示，新增版本记录当前操作人，已有版本缺省时使用数据集创建人兜底。', '根据本轮需求，数据详情页支持删除当前版本内单条数据，删除前二次确认并明确不可恢复。', '根据本轮需求，数据管理上传文件大小放开前端限制。', '根据本轮需求，训练数据列表支持编辑名称但不展示描述；详情页名称可编辑，描述按当前版本维度编辑，保存后走数据服务 store/API 统一同步；详情页改名后自动替换当前路由。', '根据本轮 V1.14 需求，训练/验证数据详情支持合并同一数据集内多个处理完成版本，未发布版本也可参与，合并结果作为最新未发布版本。', '根据本轮发布态需求，新建和新增版本默认未发布；发布入口只在详情页当前版本操作区展示，列表行代表整个数据集不展示发布按钮；未发布版本允许删除单条数据，已发布版本展示去训练入口并锁定单条删除，标注和清洗只允许选择已发布数据。根据本轮批注，修复旧数据补齐版本无法真实合并的问题，合并成功后新版本必须立即展示并可刷新保留。', '根据本轮需求，训练/验证/测试数据管理仅保留 SFT、DPO 与 RFT-GRPO 数据用途、筛选项和演示数据。', '根据本轮需求，版本状态先判断处理状态：处理中和处理失败直接展示处理状态；处理成功后才展示未发布/已发布；处理失败版本的基本信息发布状态显示为 -，且不可发布。', '根据 V1.14 缺口迁移，训练/验证/测试数据集删除接入生产引用保护：列表和详情页删除前检查标注/清洗占用，删除接口返回训练、推理、评估等引用阻断原因时直接展示给用户。'],
      syncMissingDefaultRequirements: true,
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              ...
              '每条合并明细需要在后端保留来源关系，前端数据详情表不展示来源字段，避免列表信息过载。',
            ],
            modules: [
              '训练数据管理详情页。',
              '验证数据管理详情页。',
        ...
        },
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              ...
              '发布状态只属于具体版本，不用一个数据集外层状态代表所有版本。版本先经历处理中、处理成功或处理失败；只有处理成功后才进入未发布/已发布判断。',
              '未发布版本可继续删除单条数据；已发布版本明细锁定。处理失败版本不能发布，基本信息中的发布状态显示为 -。',
            ],
            modules: [
              '训练数据管理列表、详情页和版本侧栏。',
              '验证数据管理列表、详情页和版本侧栏。',
            ...
            acceptance: [
              ...
              '处理失败版本在详情页版本侧栏显示处理失败，基本信息状态显示处理失败，发布状态显示 -。',
            ],
            interactions: [
              '详情页版本状态按处理状态优先展示：处理中/处理失败直接显示处理状态，处理成功后显示未发布或已发布，避免出现历史版本或已归档等额外状态。',
            ],
          }),
        },
        {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '版本信息', location: '展开行/详情', type: '版本列表', required: '否', description: '记录各测试版本与说明；版本状态优先展示处理状态，处理成功后才展示发布状态，处理失败时发布状态显示 -。' },
      ],
      actions: [
        ...
        { name: '合并版本', entry: '详情页左侧版本区合并版本按钮', precondition: '同一测试数据集内至少存在 2 个处理完成版本', successFeedback: '生成新的最新未发布版本，并在后端保留来源关系', errorFeedback: '版本不足或接口异常时提示' },
      ...
      states: [
        ...
        { name: '处理失败', meaning: '测试数据版本处理失败，不能进入发布可用态', presentation: '详情页版本状态为处理失败，发布状态字段为 -', availableActions: '查看失败版本信息' },
      ],
      interactionNotes: ['页面头部不再展示面包屑，统一只保留标题、返回按钮和必要操作。', '去掉面包屑后，列表页头部采用克制的标题 + 说明结构，不新增统计卡；创建页、详情页和新增版本页统一改为“返回 + 标题 + 说明/操作”的紧凑结构。', '详情页顶部标题只展示数据集名称，不提供编辑入口；名称编辑统一放在基本信息的数据集名称字段。', '版本信息需要与列表信息联动展示。', '列表表格统一使用普通横向滚动，不再使用固定操作列，避免内容与操作区重叠。', '列表页只允许编辑测试数据集名称，不新增描述列；描述属于版本维度，只在详情页当前版本基本信息中编辑。', '列表行代表整个测试数据集，不代表某个具体版本；因此列表操作不展示发布按钮，版本发布统一进入详情页后针对当前版本操作。', '字段进入编辑态后不展示叉和勾按钮，按 Enter 或点击空白失焦即保存，Esc 取消本次编辑；详情页名称和描述字段后常显编辑按钮，编辑图标紧跟字段文本，不拉到单元格或字段容器右侧。', '详情页采用左侧版本切换、右侧信息与明细展示的结构，版本区按处理状态优先展示：处理中/处理失败直接显示处理状态，处理成功后显示未发布或已发布。', '测试数据创建和新增版本的文件上传需展示进度，上传中可取消，失败或取消后可继续上传并保留断点进度；文件大小不设前端限制。', '创建页提交时如果存在未填写必填项，需要自动滚动并聚焦到首个错误字段，同时提示用户补充必填项。', '根据用户最新要求，测试数据管理的数据用途从“文本生成/图像理解 + SFT/DPO/RFT 二级选择”收敛为单级枚举；创建页、列表筛选、列表列、详情页和新增版本页均只展示文本生成/图像理解。', '历史 mock 数据中带 SFT、DPO、RFT 前缀的数据用途在测试数据页按文本生成/图像理解归一展示。', '版本合并与新增版本放在同一版本操作区域；只要求源版本处理完成，不要求已发布，因此未发布但处理完成的版本也可以参与合并。', '合并生成的新版本默认为未发布，源版本保持不变，重复样本全部保留，并通过后端来源关系追踪数据来源；旧数据补齐的版本也必须参与真实持久化合并，成功后页面立即展示且刷新保留。', '未发布版本可继续删除单条数据；发布后的版本锁定明细，不再展示单条删除操作。'],
      productionComparison: ['当前页以用户迭代需求为基线，本轮不参考生产环境。'],
      userChanges: ['根据用户最新反馈，测试数据管理创建页的数据用途只保留文本生成和图像理解，不再提供二级选择；列表和详情页的数据用途字段同步改为单级展示。', '根据本轮需求，被推理、标注、清洗或训练任务引用的测试数据集暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。', '根据本轮需求，测试数据详情页支持删除当前版本内单条数据，删除前二次确认并明确不可恢复。', '根据本轮需求，数据管理上传文件大小放开前端限制。', '根据本轮需求，测试数据列表支持编辑名称但不展示描述；详情页名称可编辑，描述按当前版本维度编辑，保存后走数据服务 store/API 统一同步；详情页改名后自动替换当前路由。', '根据本轮 V1.14 需求，测试数据详情支持合并同一数据集内多个处理完成版本，未发布版本也可参与，合并结果作为最新未发布版本。根据本轮批注，测试数据新增版本在继承历史版本开启时仍允许点击上传文件追加内容，且旧数据补齐版本合并后必须真实落盘。', '根据本轮发布入口反馈，测试数据列表代表整个数据集，不再展示发布按钮；版本发布入口保留在详情页当前版本操作区。', '根据本轮需求，测试数据版本状态同步按处理状态优先展示；处理失败版本发布状态显示为 -，且不可发布。'],
      syncMissingDefaultRequirements: true,
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              ...
              '发布状态只属于具体版本，测试数据集外层列表不展示单一发布状态列，避免用一个状态代表所有版本。版本先经历处理中、处理成功或处理失败，只有处理成功后才进入未发布/已发布判断。',
            ],
            modules: [
              '测试数据管理列表、详情页和版本侧栏。',
            ...
            acceptance: [
              ...
              '下游选择按版本展开，已发布版本可选，未发布版本展示但置灰不可选。',
              '如果最新版本未发布，但历史版本已发布，下游仍可选择历史已发布版本。',
              '处理失败版本在详情页版本侧栏显示处理失败，基本信息发布状态显示 -，且不可发布。',
            ],
            interactions: [
              '详情页版本状态按处理状态优先展示：处理中/处理失败直接显示处理状态，处理成功后显示未发布或已发布。',
            ],
          }),
        },
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              ...
              '合并后生成新的未发布版本，源版本不删除、不改写，重复样本全部保留；后端记录每条明细来源关系，前端数据详情表不展示来源字段。',
            ],
            modules: [
              '测试数据管理详情页。',
              '版本侧栏、新增版本与合并版本操作区。',
            ...
            acceptance: [
              '同一测试数据集内至少 2 个处理完成版本时，可发起版本合并；未发布但处理完成的版本也允许参与。',
              '合并成功后生成新的最新未发布版本，源版本仍保留且内容不被改写。',
              '合并样本不去重，内容或样本名重复时也全部保留。',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '导入数据用途', location: '导入推理结果集创建页', type: '单选枚举', required: '是', description: '导入推理结果集时支持文本生成 / SFT、文本生成 / DPO、文本生成 / RFT-GRPO、图像理解四类用途；提交到生产接口时仍映射为 text-generation 或 image-understanding。' },
        { name: '导入数据格式', location: '导入推理结果集创建页', type: '单选枚举', required: '是', description: 'SFT 支持 prompt-response 和 role-based；DPO 支持 alpaca 和 role-based；RFT-GRPO 支持 completion-reward；图像理解支持 role-based。' },
      ...
      interactionNotes: ['结果集用于标准产品链路，不承接业务推理结果集能力。', '列表页有行内操作和更多菜单，详情页补业务动作区。', '详情页顶部标题只展示结果集名称，不提供编辑入口；名称编辑统一放在基本信息的数据集名称字段。', '创建页中的推理方式会影响表单字段，导入推理结果集切换为上传方式、模型名称、数据用途、数据格式、上传文件和示例模板下载，不展示任务定时配置。', '导入推理结果集的数据用途按 V1.14 细分为文本生成 / SFT、文本生成 / DPO、文本生成 / RFT-GRPO、图像理解；前端根据用途约束可选数据格式，提交时仍使用生产后端 dataset_type 与 dataset_format 字段。', '导入推理结果集的示例模板下载会把当前导入数据用途同步传给后端；DPO role-based 不再复用 SFT role-based 样例，RFT-GRPO 使用 completion-reward 样例。', '导入推理结果集的文件上传需展示进度，上传中可取消，失败或取消后可继续上传并保留断点进度；普通文本生成导入只允许 jsonl/json/xlsx，业务推理额外允许 csv，图像理解只允许 zip。', '创建页删除独立“数据用途”和“数据量”录入字段，数据量由选择的数据集版本自动带出。', '待推理数据改为只读输入框 + 选择弹窗，可选择数据集；训练/验证数据集只允许选择 SFT 类型版本，DPO/RFT 数据在数据源层面过滤，弹窗的数据用途筛选展示“全部、文本生成、图像理解”，不单独展示 SFT 选项。', '待推理模型改为级联选择：模型仓库/我的模型 -> 模型 -> 版本；选择后展示推理模型参数设置。', '推理模型参数的数值输入与 -/+ 操作按钮采用不换行栅格，避免窄屏下按钮错位。', '在线推理不展示显卡资源配置，离线推理才展示显卡资源配置。', '创建页删除底部“当前推理方式/待推理来源”摘要卡。', '创建页提交时如果存在未填写必填项，需要自动滚动并聚焦到首个错误字段，同时提示用户补充必填项。', '全局任务流转需遵守统一状态规则，尤其是启动中任务不允许终止。', '当前页以用户迭代需求为基线；生命周期任务支持在列表名称列与描述列分别行内编辑，编辑入口悬浮显示且不受任务状态影响，效果评估引用的推理结果集选项随数据服务任务源刷新。', '列表与详情字段进入编辑态后不展示叉和勾按钮，按 Enter 或点击空白失焦即保存，Esc 取消本次编辑；详情页名称和描述字段后常显编辑按钮，编辑图标紧跟字段文本，不拉到单元格或字段容器右侧。'],
      productionComparison: ['本轮按用户要求直接访问生产环境推理结果集创建页核对，创建表单对齐其推理方式、定时配置、待推理数据选择和资源配置结构；用户明确要求删除的数据用途字段已改为弹窗筛选项。'],
      userChanges: ['根据本轮批注，推理创建页删除独立数据用途和数据量字段，待推理数据改为弹窗选择，并在弹窗中补充 SFT/DPO/RFT-GRPO 数据用途筛选。', '根据最新批注，补齐待推理模型级联选择、模型参数设置、数据弹窗整行点击展开、在线推理隐藏资源配置、导入推理结果集上传配置和模板下载。', '根据本轮批注，修复推理模型参数 -/+ 按钮在窄区域换行错位问题。', '根据本轮需求，推理结果集详情页支持删除单条推理明细，删除前二次确认并提示不可恢复。', '根据本轮全局任务需求，推理结果集列表支持名称列与描述列分别行内编辑，编辑入口悬浮显示且不受任务状态影响，保存后通过数据服务 store/API 同步到评估选择项和引用提示。', '根据本轮需求，数据管理上传文件大小放开前端限制。', '根据本轮需求，推理结果集待推理数据保留测试数据集；训练/验证数据集中的 DPO/RFT 数据从选择弹窗的数据源、筛选项、列表和确认结果中统一过滤，并补充限制提示与空状态。', '根据本轮需求，推理结果集详情页同步支持名称和描述行内编辑，详情保存后与列表共享同一状态源。', '根据本轮 V1.14 生产替换迁移，导入推理结果集的数据用途细分为文本生成 / SFT、文本生成 / DPO、文本生成 / RFT-GRPO、图像理解，并补齐生产后端 DPO alpaca、DPO role-based 与 RFT-GRPO completion-reward 导入解析和示例模板下载。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', '当前生产替换期已按生产训练任务 schema 承接 RFT-GRPO 训练方法、数据选择过滤、GRPO 参数持久化、训练创建页模板选择、版本详情 YAML 快照展示、奖励函数上传和三阶段资源配置。', '旧结构：基础模型创建页按固定 Qwen 系列筛选模型版本并直铺在创建页；新结构：模型仓库选项改为弹窗，弹窗内展示模型仓库全部模型，并展示适配状态。', '模型配置模块改名为基础模型配置；模型来源为模型仓库时，具体选择字段命名为模型仓库选项，不再继续叫基础模型版本。', '根据本轮需求，创建页板块顺序调整为基础配置、基础模型配置、训练配置、数据配置，用户先选模型再配置训练参数。', '模型来源为我的模型时同样使用弹窗选择：创建页仅展示选择入口和已选摘要；弹窗左侧先按训练类型筛选文本生成/图像理解，再按训练方法筛选全部/SFT/DPO/RFT，右侧模型列表中选择具体版本。', 'Qwen 系列进入已适配名单判断：已下载模型展示“已适配”且允许选择；未下载模型展示“已适配 + 未下载，请联系管理员”且不可选择。', '非 Qwen 系列或不在已适配名单中的模型只展示“未适配”状态，仍允许选择，不额外展示其它提示。', '弹窗标题行不展示“已适配/未适配”状态图例，适配状态仅保留在具体模型卡片上。', '训练配置新增 DeepSpeed 训练加速配置，默认关闭；开启开关后以 Radio 单选形式展示 ZeRO-0、ZeRO-2、ZeRO-3，界面不展示内部配置文件路径。', '训练加速配置标题悬浮说明需要保持精简，只说明 DeepSpeed 用于提升训练效率和优化显存占用。', '训练加速配置采用紧凑单选排版，不使用按钮组或大卡片；每个 ZeRO 类型展示短说明，并通过问号悬浮提示查看详细注释。', '训练加速配置开关旁不展示额外说明文字，避免创建页信息过密。', '训练加速配置展示在模板管理上方。', 'GRPO 场景下，“训练参数配置”支持选择系统配置中已启用的训练参数模板：模板选择后回填为 GRPO 专属参数表单并允许本次修改，提交时保存模板快照与最终应用参数，不反写系统模板。', '模板模式下不展示额外的已选择提示卡，用户通过选择框和下方参数表单识别当前配置。', 'GRPO 模板模式下，fineTuneType 作为参数模板中的“参数类型”分组展示，不再单独拎出为独立配置段。', 'GRPO 参数表单使用参数类型、训练控制、生成采样、策略优化、数据处理、评估保存和 LoRA 配置等专属分组，不复用 SFT 参数分组。', 'GRPO 参数模板不包含 RFT 算法、奖励规则、DeepSpeed 训练加速、模型、数据集、资源规格或定时启动。', 'GRPO 奖励规则配置只开放自定义代码方式，不展示奖励规则类型名称和预设规则列表，直接展示上传 .py 文件、下载参考模板和函数约束。', 'GRPO 自定义奖励函数上传使用生产分片上传组件，提交时只保存 uploadId、文件名和文件地址引用，不把 Python 代码写入参数模板。', 'GRPO 自定义奖励函数的参考模板以 .py 文件下载提供，页面只展示下载入口和上传约束，不再直接展示模板代码。', 'GRPO 场景下，显卡资源配置拆为 Hand、Work、Submit 三个 Tab；Hand 和 Work 配置显卡、CPU、内存，Submit 只配置 CPU 和内存，三个阶段都需要填写。', 'GRPO 三阶段资源配置保存到 additional_params.grpo_resource_config；生产原 graphics_card_resource 仍保留为执行兜底，Work 阶段作为其优先来源。', '旧 RFT/GRPO 版本如果缺少三阶段资源配置，版本详情也必须展示 Hand、Work、Submit，并用原单组资源信息兜底，避免回退到旧单表格。', 'ZeRO-0/2/3 选项必须提供悬浮说明：ZeRO-0 是普通 DDP 基线，ZeRO-2 切分优化器状态和梯度，ZeRO-3 切分优化器状态、梯度和参数。', '版本详情的任务基本信息需展示当前版本训练加速配置，缺省或关闭展示“未开启”，开启时展示 ZeRO-0、ZeRO-2 或 ZeRO-3。', '版本详情的参数配置 Tab 在 GRPO 场景下直接展示本次任务保存的 YAML 参数模板内容，不再拆成基础参数/高级配置等子 Tab。', '版本详情的显卡资源配置在 GRPO 场景下同样按 Hand、Work、Submit 三个 Tab 展示。', '训练任务详情页的任务版本表格设置明确列宽与横向滚动，小屏下可右滑查看创建时间后的操作列。', '创建图像理解训练任务时，训练数据集选择弹窗提供 SFT、DPO、RFT-GRPO 场景下可演示的图像理解训练数据。', 'RFT 算法选择暂时只开放 GRPO。', '创建页提交时如果存在未填写必填项，需要自动滚动并聚焦到首个错误字段，同时提示用户补充必填项。', '根据本轮批注，图像理解 SFT 数据集选择弹窗的格式筛选不展示 PROMPT_RESPONSE，只展示 ROLE_BASED。', '训练任务从 mockTasks 直读改为共享本地 store；列表仅保留任务名称行内编辑，任务描述不再作为训练列表和任务信息字段展示。'],
      productionComparison: ['生产代码已覆盖训练任务主链路；本次在生产 schema 与 additional_params 扩展口上补齐 RFT-GRPO 枚举、GRPO 参数保存、训练创建页模板快照、奖励函数上传引用、三阶段资源快照和版本详情 YAML 快照展示。', '生产替换期已确认：GRPO 参数模板不能直接使用 Demo localStorage 模板中心；当前已补生产后端持久化、系统设置 CRUD、训练创建页模板选择、奖励函数生产分片上传、三阶段资源配置和版本详情快照。'],
      userChanges: ['根据本轮生产替换盘点，生产训练主链路已作为基线保留；本次先补齐 RFT-GRPO 训练方法枚举、创建页 GRPO 参数表单、数据集选择过滤、additional_params.grpo_config 持久化、训练创建页模板快照和版本详情 YAML 快照展示。', '根据本轮需求，大模型训练创建页的模型仓库选项改为展示模型仓库全部模型；Qwen 作为已适配名单示例，已下载可选，未下载提示“已适配 + 未下载，请联系管理员”且不可选；其它系列只展示未适配状态但仍可选。', '根据本轮调整，模型仓库选项改成弹窗：先选模型提供商，再选具体模型，避免在创建页直铺大量模型。', '根据本轮需求，大模型训练版本详情中的显卡资源配置字段与创建页所选字段保持一致，并展示 CPU/内存请求与限制。', '根据用户截图反馈，修复训练任务详情小屏下任务版本表格无法右滑、右侧操作列不可见的问题。', '根据本轮演示需求，补充创建图像理解训练任务时可选择的多模态训练数据集。', '根据本轮批注，图像理解 SFT 数据集格式收敛为 ROLE_BASED，避免在创建训练任务时出现 PROMPT_RESPONSE。', '根据本轮全局任务需求，大模型训练任务补任务级描述并支持名称/描述行内编辑，版本级 taskDescription 保持原语义。', '根据本轮上传的 DeepSpeed 配置文档，创建页新增训练加速配置，可选不开启、ZeRO-0、ZeRO-2、ZeRO-3，并为各策略提供悬浮注释。', '根据本轮反馈，训练加速配置改为微调类型上方的开关控制项，打开后展开 ZeRO-0/2/3，微调类型下方继续紧跟参数配置。', '根据本轮批注，精简训练加速配置 tooltip，并修复开关关闭态下无法开启的问题。', '根据本轮批注，训练加速配置改为紧凑一行排版，开启后仅展示 ZeRO 策略按钮，详细说明通过悬浮提示查看。', '根据本轮反馈，ZeRO 策略选择从按钮组调整为 Radio 单选项，每个类型保留短说明和问号悬浮注释。', '根据本轮批注，删除训练加速配置开关旁的“启用 DeepSpeed”文字。', '根据本轮需求，版本详情页的任务基本信息同步展示创建页训练加速配置，缺省为未开启，开启时展示对应 ZeRO 策略。', '根据本轮截图反馈，大模型训练列表和任务信息删除任务描述字段，仅保留训练任务名称、训练类型、版本数量、创建时间和操作等核心列。', '根据本轮新增需求，创建页“模型配置”模块改名为“基础模型配置”，模型仓库来源下的具体选择字段改名为“模型仓库选项”。', '根据本轮批注，模型来源选择“我的模型”时改为弹窗选择，弹窗内按文本生成/图像理解筛选，并在模型列表中选择具体版本。', '根据本轮批注，我的模型选择弹窗左侧新增训练方法筛选，支持全部、SFT、DPO、RFT 与训练类型组合过滤。', '根据本轮需求，RFT 算法只展示 GRPO，我的模型和数据集选择中同步隐藏 PPO 演示项。', '根据本轮需求，GRPO 创建页字段名改为“训练参数配置”，支持“模板管理”和“自定义参数”两种方式；模板选择后不展示额外提示卡，参数类型作为 GRPO 参数表单的一个分组展示，自定义参数只展示 YAML 编辑区。', '根据本轮需求，GRPO 参数表单改为专属分组，覆盖生成采样、策略优化等 GRPO 常用参数，不再复用 SFT 参数分组。', '根据本轮需求，GRPO 奖励规则配置只保留自定义代码操作页，不展示奖励规则类型名称和预设规则列表。', '根据本轮需求，GRPO 的显卡资源配置拆为 Hand、Work、Submit 三个阶段，Submit 只配置 CPU 和内存。', '根据本轮需求，创建页基础模型配置与训练配置对调，进入创建页后先选模型，再设置训练类型、训练方法和参数。', '根据本轮需求，GRPO 自定义奖励函数参考模板从页面内代码展示改为 .py 文件下载。', '根据本轮生产替换迁移，RFT-GRPO 创建页可选择系统配置中已启用的训练参数模板，模板参数回填到当前表单，提交时保存模板 YAML 快照和本次最终应用参数，不反写系统模板。', '根据本轮生产替换迁移，RFT-GRPO 创建页奖励函数上传已改用生产分片上传组件，提交时把 uploadId、文件名和文件地址保存到 additional_params.grpo_reward_function。', '根据本轮生产替换迁移，RFT-GRPO 创建页新增 Hand / Work / Submit 三阶段资源配置，提交时保存到 additional_params.grpo_resource_config，Work 阶段同步作为生产 graphics_card_resource 的优先来源。', '根据本轮生产替换迁移，RFT-GRPO 版本详情参数配置新增 GRPO模板页签，优先展示训练任务保存的 YAML 快照，旧版本无模板快照时按 grpo_config 生成只读兜底内容。'],
      syncMissingDefaultRequirements: true,
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      actions: [
        ...
        { name: '删除服务', entry: '列表行操作', precondition: '服务未被业务链路引用', successFeedback: '服务从列表移除', errorFeedback: '被推理结果集、评估、基准评估或自动标注引用时展示后端阻断原因' },
      ],
      states: [
      ...
      interactionNotes: ['在线推理服务与大模型部署是两类不同服务，不再共用同一份设计文档。', '新增 /service/inference/external/create 路由，用于从系统配置 Agent助手中跳转创建对话模型服务；创建或取消后按 redirect 参数返回来源页。', '新建或编辑服务提交时如果存在未填写必填项，需要自动滚动并聚焦到首个错误字段，同时提示用户补充必填项。', '在线推理服务列表名称列和描述列统一接入字段级行内编辑，编辑入口仅在鼠标悬浮对应字段时展示，点击后当前字段变为输入框。', '生产替换期删除保护已由后端硬校验承接：在线推理服务若被推理结果集、评估任务、评估报告、基准评估或自动标注配置引用，删除时返回 409 阻断原因。'],
      productionComparison: ['以当前生产环境在线推理服务列表页为基线；V1.14 引用保护只补生产后端删除前校验，不迁入 Demo 本地引用表。'],
      userChanges: ['根据本轮需求，在线推理服务可作为文档中心 Agent 的对话模型来源，并支持从 Agent 配置页跳转新建后返回。', '根据本轮需求，在线推理服务补齐删除操作；若被推理或标注任务引用则暂不允许删除，任务完成不释放引用，必须删除任务或停止引用链。', '根据本轮全局编辑反馈，在线推理服务名称和描述补齐悬浮行内编辑，且不受连接状态影响。', '根据生产替换迁移，在线推理服务删除引用保护改为生产后端 409 硬拦截，覆盖推理结果集、评估任务、评估报告、基准评估和自动标注配置引用。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      actions: [
        ...
        { name: '合并版本', entry: '详情页左侧版本区合并版本按钮', precondition: '同一机器学习数据集内至少存在 2 个处理完成版本', successFeedback: '提交后生成递增版本并自动切换到新版本，后端保留来源关系', errorFeedback: '版本不足或提交异常时提示' },
      ...
      interactionNotes: [
        ...
        '版本合并与新增版本放在同一版本操作区域；机器学习数据管理已接入生产后端真实合并接口，合并后生成新版本并写入持久化存储，未发布概念不阻断合并。',
      ...
      userChanges: ['保留用户明确要求：创建数据集补版本字段，默认 V1；数据类型移除音频，并将文本排序在图片前。', '根据本轮需求，被在线标注任务引用的机器学习数据集暂不允许删除；任务完成不释放引用，必须删除任务或停止引用链。', '根据本轮需求，机器学习数据集补充创建人字段，并限制仅创建人本人、项目管理员或租户管理员可删除。', '根据本轮要求重新参考生产环境，机器学习数据管理详情改为整页版本侧栏 + 基本信息 + 数据详情表结构，并支持删除详情表中的单条数据。', '根据本轮计划，基本信息样式改为生产环境式标签-值网格，新增版本从占位提示改为可提交的新版本表单闭环。', '根据本轮需求，机器学习数据管理上传文件大小放开前端限制。', '根据本轮 V1.14 需求，机器学习数据管理详情支持合并同一数据集内多个版本，合并结果作为最新版本并在后端保留每条样本来源关系；本轮已从 Demo 本地闭环迁入生产接口和持久化存储。', '根据本轮发布态需求，机器学习数据新建/新增版本默认未发布；发布入口只在详情页当前版本操作区展示，列表行代表整个数据集不展示发布按钮；未发布版本允许单条删除，已发布版本锁定单条删除，数据标注仅选择已发布数据。', '根据本轮图像分割拆分需求，图片-图像分割下新增实例分割（标准）、实例分割（孔洞）和语义分割三种标注模板，回形带孔数据归入实例分割（孔洞）。', '根据本轮补充需求，创建页上传数据区域增加数据模板展示和下载，实例分割（标准）、实例分割（孔洞）和语义分割分别下载对应模板。'],
      syncMissingDefaultRequirements: true,
        ...
        },
        {
          ...
          content: formatRequirementDoc({
            ...
            acceptance: [
              ...
              '合并版本基本信息中的数据来源展示为“版本合并”，每条样本在后端持久化时记录 source_version 作为来源关系。',
            ],
            interactions: [
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            interactions: [
              ...
              '生产替换阶段暂不以 Demo 本地权限模型落地该强校验；如进入 V1.14 必验范围，需要由生产后端提供正式资源归属字段、IAM 角色/资源范围接口和统一权限拒绝响应后再迁移。',
            ],
          }),
        },
      ],
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', '模型仓库页面已按用户提供的全局任务流转规则收口。', '旧结构：模型提供商收敛为 Qwen，列表、表单和详情包含模型类型与支持能力字段；新结构：模型类型和支持能力均从列表、表单、详情和数据结构中删除，模型提供商放开为国内外主流开源提供商并支持自定义。', '新增模型弹窗保留“本地 / ModelScope”来源切换：本地来源使用模型 Code 下拉或自定义输入，ModelScope 来源补集群选择、下载入口提示和任务定时配置。', '筛选区支持换行，列表设置明确列宽与横向滚动，避免小屏内容错位或右侧操作不可见。', '生产替换期删除保护已由后端硬校验承接：基础模型、训练模型和机器学习模型若被训练、部署、推理结果集、评估任务、评估报告或基准评估引用，删除时返回 409 阻断原因。'],
      productionComparison: ['以生产模型管理实现为基线；V1.14 引用保护只补生产后端删除前校验，不迁入 Demo 本地引用表。'],
      userChanges: ['根据待迭代项截图，本地上传模型操作中不再展示日志能力，新增模型补模型来源选择，支持本地和 ModelScope。', '根据本轮批注重做新增基础模型弹窗字段顺序和来源差异，并修复模型仓库小屏错位与无法横向滑动问题。', '根据最新批注将模型类型从 LLM/VLM 改为文本生成/图像理解，并将模型提供商收敛为 Qwen。', '根据本轮需求，模型仓库放开模型提供商限制，下拉支持国内外主流开源提供商和自定义，同时删除支持能力字段。', '根据本轮需求，模型仓库继续删除模型类型字段，训练创建页改为根据模型 Code/名称推断训练类型。', '根据本轮需求，被推理、部署或训练任务引用的模型仓库条目暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。', '根据生产替换迁移，模型删除引用保护改为生产后端 409 硬拦截，覆盖训练任务、训练模型产物、模型部署、推理结果集、评估任务、评估报告和基准评估引用。'],
      syncMissingDefaultRequirements: true,
      defaultRequirements: [
        {
          version: 'V1.14',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      productionComparison: ['生产系统配置当前已覆盖属性配置与标签配置。', '生产替换期已确认：模板管理不能直接迁入 Demo localStorage；当前已以生产后端持久化模板配置为真相源补齐系统设置入口。'],
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      structure: ['属性配置 Tab', '标签配置 Tab', '训练参数模板 Tab', '标签配置左侧分组', 'Agent助手 Tab', '文档中心单目录', 'Agent服务单配置卡', 'Agent 服务配置弹窗', '标签值弹窗'],
      fields: [
        ...
        { name: '大模型训练参数配置', location: '训练参数模板 Tab', type: '参数配置列表 + YAML 编辑弹窗', required: '否', description: '把 RFT-GRPO 训练参数沉淀为可复用配置；新增或编辑时维护模板名称、训练方法、描述和 YAML 参数模板。' },
      ...
      actions: [
        ...
        { name: '维护大模型训练参数配置', entry: '训练参数模板 Tab', precondition: '进入系统配置', successFeedback: '配置保存到生产模板接口', errorFeedback: '模板字段越界或基础信息校验失败时提示' },
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', '当前页已根据用户需求把“添加标签”和“添加标签值”拆成两层操作。', '标签配置旧结构：左侧仅在线Notebook / 自定义镜像；新结构：在线Notebook和模型仓库保持同一顶层层级，自定义镜像缩进为在线Notebook子项。', '添加属性按钮已接入新增属性弹窗，属性列表移除“输入方式”列，仅展示属性名称、属性描述、属性值、属性分组和是否必填。', '大模型训练参数配置用于沉淀团队内可复用的训练参数口径，降低创建任务时重复填写和误填参数的概率。', '生产替换期顶部 Tab 展示为“训练参数模板”，直接展示大模型 RFT-GRPO 参数模板列表。', '列表展示训练方法、参数类型、状态和更新时间，训练方法当前只展示 RFT-GRPO。', '训练参数模板只管理参数类型和训练参数，不能配置奖励规则、DeepSpeed、模型、数据、资源或任务信息。', '配置编辑弹窗中，模板名称下方提供训练方法字段，当前下拉只允许选择 RFT-GRPO。', '配置编辑弹窗不提供本地分组表单，用户补充模板名称、训练方法和描述后，通过 YAML 模板维护完整参数内容。', '模板内容采用 YAML 结构，根字段 fineTuneType 定义参数类型，params 定义对应训练参数，params 只允许当前产品已有训练参数字段。', '配置停用后不再进入后续大模型训练创建页可选列表，历史任务展示仍读取任务保存的配置名称。', 'Agent助手当前以用户迭代需求为基线，不参考生产环境；服务作用域固定为全局文档中心。', 'Agent助手左侧目录只展示一个“文档中心”，右侧标题使用“Agent服务”。', 'Agent 服务从可配置多个调整为只可配置一个；配置保存完成后即自动启动服务。', '对话模型可选择在线推理服务，也可自行配置 API；如果没有可用在线推理服务，可跳转在线推理服务创建页。', '配置弹窗底部不展示 Embedding 修改提示，只在实际保存且 Embedding 配置变化时弹窗确认是否重建向量。', '对话模型参数收进“高级配置”，默认不展开。'],
      productionComparison: ['生产系统设置已覆盖属性配置、标签配置；V1.14 迁移已补训练参数模板 Tab，并以生产后端 `training_parameter_templates` 作为模板真相源。', '训练创建页已读取已启用模板并保存任务级模板快照，训练版本详情已展示 YAML 快照；奖励函数上传已在训练创建页通过生产分片上传通道承接。'],
      userChanges: ['根据待迭代项截图，添加标签仅添加标签，新增标签值需要进入“查看标签值”后处理。', '根据本轮批注修复添加属性按钮无响应问题，并移除属性列表的输入方式字段。', '根据本轮需求新增 Agent助手 Tab：在文档中心目录下配置全局 Agent 服务，拆分 Embedding、Rerank 和对话模型配置，并接入最小 RAG 接口契约。', '根据浏览器批注删除 Agent 页左侧“标签配置”标题，左侧只展示一个文档中心，右侧改为单 Agent 服务配置卡并解决表格列重叠。', '根据本轮批注，Agent 页顶部说明改为功能描述，对话模型参数改为默认收起的高级配置，并删除弹窗底部 Embedding 提示卡。', '根据本轮需求，标签配置左侧新增模型仓库一级选择，模型仓库标签与在线Notebook标签按业务域分组展示。', '根据本轮反馈，左侧调整为在线Notebook和模型仓库同级，自定义镜像作为在线Notebook子项。', '根据本轮需求，系统配置新增模板管理 Tab，用于沉淀团队可复用的大模型训练参数口径，并在创建任务时快速带出。', '根据生产替换迁移，系统配置新增训练参数模板 Tab，模板列表、新增、编辑、复制、删除和启停均接入生产后端模板接口，不再使用 Demo localStorage 模板中心。', '根据本轮生产替换迁移，启用的 RFT-GRPO 训练参数模板已可在大模型训练创建页选择，停用模板不会进入创建页可选列表。'],
      syncMissingDefaultRequirements: true,
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            acceptance: [
              ...
              '模板保存、启停、复制和删除均通过生产模板接口完成，不再写入 Demo localStorage。',
              '新增或编辑配置后，可在大模型训练创建页 RFT-GRPO 场景选择已启用模板。',
              '停用配置不出现在创建页可选列表。',
              'YAML 根字段只允许 `fineTuneType` 和 `params`；输入奖励规则、DeepSpeed、模型、数据或资源字段时需要校验失败。',
            ],
            interactions: [
              '当前生产适配先保留 YAML 编辑；训练创建页已接入模板选择，版本详情已展示 YAML 快照，表单预览作为后续体验优化继续迁移。',
            ],
          }),
        },
      ],
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['生产替换阶段已以生产系统管理代码作为基线，项目、用户、成员、集群、存储、镜像仓库、基础模型、系统设置和权限主链路均优先采用生产实现。', 'V1.14 Demo 中仍未被生产代码直接覆盖的系统管理能力，需要先登记覆盖矩阵，再按生产接口和权限体系最小适配。'],
      productionComparison: ['当前已对齐生产系统管理主链路；Demo 本地系统管理 store、localStorage 权限模型和孤立 mock 页面不直接迁入生产代码。'],
      userChanges: ['根据生产代码替换盘点，系统管理总说明从 Demo/mock 基线调整为生产代码基线，并把权限资源范围标记为 partial-conflict 缺口。'],
    }),
  },
]
