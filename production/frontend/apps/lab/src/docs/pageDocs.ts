...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      productionComparison: ['生产系统设置已覆盖属性配置、标签配置；V1.14 迁移已补训练参数模板 Tab，并以生产后端 `training_parameter_templates` 作为模板真相源。', '训练创建页已读取已启用模板并保存任务级模板快照；奖励函数上传和版本详情 YAML 展示仍属于后续 GRPO 缺口。'],
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            interactions: [
              '当前生产适配先保留 YAML 编辑；训练创建页已接入模板选择，表单预览和版本详情 YAML 展示作为后续 GRPO 缺口继续迁移。',
            ],
          }),
        },
      ],
