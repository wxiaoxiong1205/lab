...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      productionComparison: ['生产系统设置已覆盖属性配置、标签配置；V1.14 迁移已补训练参数模板 Tab，并以生产后端 `training_parameter_templates` 作为模板真相源。', '训练创建页已读取已启用模板并保存任务级模板快照，训练版本详情已展示 YAML 快照；奖励函数上传仍属于后续 GRPO 缺口。'],
