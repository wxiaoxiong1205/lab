...
const localModelSourceOptions: ModelSourceOption[] = [
  { label: 'ModelScope', value: 'ModelScope' },
  { label: 'Local', value: 'Local' },
]

const localModelStatusOptions: ModelStatusOption[] = [
  { label: '未开始', value: '未开始' },
  { label: '下载中', value: '下载中' },
  { label: '已完成', value: '已完成' },
  { label: '失败', value: '失败' },
...
export const ModelService = {
  ...
  getMoudleSorceEnums: async () => {
    try {
      ...
      const options = pickArray<ModelSourceOption>(response.data)
      if (options) {
        return options
      }

      if (import.meta.env.DEV) {
        console.warn('本地预览：模型来源枚举不是数组，使用预览枚举兜底。', response.data)
        return localModelSourceOptions
      }
      ...
      if (import.meta.env.DEV) {
        console.warn('本地预览：模型来源枚举获取失败，使用预览枚举兜底。', error)
        return localModelSourceOptions
      }
  ...
  getBaseModelStatusEnums: async () => {
    try {
      ...
      const options = pickArray<ModelStatusOption>(response.data)
      if (options) {
        return options
      }
