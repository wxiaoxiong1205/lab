...
import { StatusMaps } from '@/components/gpustacks/config'
...
export const backendTipsList = [
  // {
  //   title: 'llama-box',
  //   tips: '用于 GGUF 格式模型，支持 Linux, macOS 和 Windows。'
  // },
  {
    title: 'vLLM',
    tips: $t('用于非 GGUF 格式模型，仅支持 Linux。'),
  },
  {
    title: 'Ascend MindIE',
    tips: $t('用于非 GGUF 格式模型，仅支持昇腾 910B 和 310P。'),
  },
  {
    title: 'DGI-Server',
    tips: $t('仅支持昇腾910B'),
...
export const localPathTipsList = [
  {
    ...
    tips: $t('指向模型文件，例如 /data/models/model.gguf'),
  },
  {
    title: {
      text: $t('分片的 GGUF 格式模型'),
      locale: false,
    },
    tips: $t('指向模型第一个分片文件，例如 /data/models/model-00001-of-00004.gguf'),
  },
  {
    title: {
      text: $t('safetensors 格式模型'),
      locale: false,
    },
    tips: $t('指向包含 .safetensors, config.json 文件的模型目录，例如 /data/models/model'),
  },
]
...
export const backendLabelMap = {
  ...
  [backendOptionsMap.dgiServer]: 'DGI-Server',
}

export const backendParamsHolderTips = {
  [backendOptionsMap.llamaBox]: {
    holder: $t('例如， --max-model-len=8192'),
...
export const modelCategories = [
  // { label: $t('自动'), value: null, locale: true },
