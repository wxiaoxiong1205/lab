...
} from '@/components/gpustacks/config'
import { queryModelFilesList, queryWorkersList } from '@/components/gpustacks/api'
...
export const useCheckCompatibility = () => {
  ...
  const handleCheckCompatibility = (
    ...
    if (error) {
      return {
        show: true,
        type: 'danger',
        message: `Evaluation Error: ${error_message}`,
      }
    }
    ...
    if (hasClaim) {
      ...
      let message = `${$t('该模型大约需要分配')} ${$t('内存')}: ${ram}, ${$t('显存')}: ${vram}`
      if (!ram) {
        message = `${$t('该模型大约需要分配')} ${$t('显存')}: ${vram}`
      }
      if (!vram) {
        message = `${$t('该模型大约需要分配')} ${$t('内存')}: ${ram}`
      }
      msgData = {
        title: $t('兼容性检查通过'),
        message,
  ...
  const updateShowWarning = (params: {
    ...
    if (isOllamaModelFile && backend === backendOptionsMap.llamaBox) {
      ...
      warningMessage = 'This is an Ollama model file. Please use LlamaBox backend.'
    }
    else if (isGGUFFile && backend !== backendOptionsMap.llamaBox) {
      warningMessage = 'GGUF models require LlamaBox backend.'
    }
    else if (!isGGUFFile && backend === backendOptionsMap.llamaBox) {
      warningMessage = 'LlamaBox backend only supports GGUF models.'
    }
