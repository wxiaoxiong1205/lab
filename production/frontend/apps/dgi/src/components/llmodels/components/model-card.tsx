...
const MarkDownTitle: React.FC<{
  ...
  return (
    <MkdTitle onClick={onCollapse}>
      <span>
        <FileMarkdownOutlined className="m-r-2 text-tertiary" />
        {' '}
        README.md
      </span>
      <span>
...
const ModelCard: React.FC<{
  ...
  const getHuggingfaceModelDetail = async () => {
    try {
      ...
      const isGGUF = (modelcard as any)?.tags?.includes('gguf')
      setIsGGUF(isGGUF)
  ...
  const generateModelLink = () => {
    ...
    if (modelSource === modelSourceMap.huggingface_value) {
      return (
        <Tooltip title="models.viewin.hf">
          <Button
            size="small"
            type="link"
            target="_blank"
    ...
    if (modelSource === modelSourceMap.modelscope_value) {
      return (
        <Tooltip title="models.viewin.modelscope">
          <Button
            size="small"
            type="link"
            target="_blank"
