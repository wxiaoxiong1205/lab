...
import SearchStyle from '../style/search-result.module.css'
import SearchInput from './search-input'
import SearchResult from './search-result'
import { createAxiosToken } from '@/hooks/use-chunk-request'
import { getRequestId, setRquestId } from '@/stores/models'
...
const SearchModel: React.FC<SearchInputProps> = (props) => {
  ...
  const getModelsFromModelscope = async (queryParams: {
    ...
    try {
      ...
      const list = _.map(_.get(data, 'Data.Model.Models') || [], (item: any) => {
        return {
          path: item.Path,
  ...
  const renderGGUFTips = useMemo(() => {
    return (
      <Tooltip
        ...
        title={(
          <UL>
            <li>GGUF models are optimized for CPU inference</li>
            <li>VLLM backend does not support GGUF models</li>
            <li>
              VoxBox backend supports GGUF models for CPU inference
            </li>
          </UL>
        )}
      >
