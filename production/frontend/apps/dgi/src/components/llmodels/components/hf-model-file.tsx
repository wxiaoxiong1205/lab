...
import '../style/hf-model-file.css'
...
const HFModelFile: React.FC<HFModelFileProps> = forwardRef((props, ref) => {
  ...
  const modelFilesSortOptions = useRef<any[]>([
    {
      label: 'models.sort.size',
      value: 'size',
    },
    {
      label: 'models.sort.name',
      value: 'name',
    },
  ])
  ...
  const getHuggingfaceFiles = async () => {
    try {
      ...
      const list = _.filter(fileList, (file: any) => {
        return hfFileFilter(file) && !file.path.includes('mmproj')
      })
        ...
        <div style={{ padding: '16px 24px' }}>
          {dataSource.loading ? (
            ...
            <Empty
              ...
              description="models.search.nofiles"
            />
          )}
        </div>
      </SimpleOverlay>
