...
import EventModelOrContent from './EventModelOrContent'
import { $t } from '@/locales'

const EventManagementPage: React.FC = () => {
  const model_connectivity = 'model_connectivity'
  const content_security = 'content_security'
  ...
  const items = [
    {
      ...
      children: (
        <div className="space-y-4">
          <EventModelOrContent type={model_connectivity} key={new Date().getTime()} />
        </div>
      ),
    },
    // {
    ...
    //       <EventModelOrContent type={model_status} key={new Date().getTime()} />
    //     </div>
    //   ),
    // },
    {
      ...
      children: (
        <div className="space-y-4">
          <EventModelOrContent type={content_security} key={new Date().getTime()} />
        </div>
      ),
    },
  ]
