...
import { queryWorkersList } from '@/components/gpustacks/api'
import type { ListItem as WokerListItem } from '@/components/gpustacks/config/types'
...
import {
  ...
  queryModelsList,
} from '@/components/llmodels/apis'
import TableList from '@/components/llmodels/components/table-list'
import { backendOptionsMap } from '@/components/llmodels/config'
import type { ListItem } from '@/components/llmodels/config/types'
import { useGenerateModelFileOptions } from '@/components/llmodels/hooks'
...
const Models: React.FC = () => {
  ...
  const chunkInstanceRequedtRef = useRef<any>(null)
  const isPageHidden = useRef(false)
  const instancesToken = useRef<any>(null)
  ...
  const fetchData
    = async (params?: {
      ...
      try {
        const currentParams = query || queryParams
        const params = {
          page: currentParams.page,
          perPage: currentParams.perPage,
          ..._.pickBy(_.omit(currentParams, ['page', 'perPage']), (val: any) => !!val),
        }
    ...
    const getTableData = async (loadingVal?: boolean) => {
      ...
      try {
        const params = {
          ...
          ..._.pickBy(_.omit(queryParams, ['page', 'perPage']), (val: any) => !!val),
        }
        const res: any = await queryModelsList(params, {
          cancelToken: axiosToken.token,
        })
  ...
  return (
    <div className="p-4 bg-white">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">{$t('我的模型')}</h1>
      </div>
