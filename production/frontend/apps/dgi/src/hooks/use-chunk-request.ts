...
import { WatchEventType } from '@/components/gpustacks/config'
import {
  cancelWatchRequest,
  clearWatchRequestId,
  updateWatchIDValue,
  updateWatchRequest,
} from '@/stores/watch-request'
...
const useSetChunkRequest = () => {
  const watchRequestList = typeof window !== 'undefined' ? window.__GPUSTACK_WATCH_REQUEST_CLEAR__.requestList : []
    ...
    http.interceptors.request.use(
      (config) => {
        ...
        if (config.url?.includes('gpustack')) {
          ...
          // 其他请求添加 /api_v2 前缀
          config.baseURL = '/api_v2'
        }

        // 从 localStorage 获取 token
        // const token = localStorage.getItem("dgi-token");
    ...
    try {
      ...
      const response = await httpInstance({
        ...
        onDownloadProgress(progressEvent: AxiosProgressEvent) {
          const xhr = (progressEvent as any).target || (progressEvent as any).currentTarget
          if (xhr) {
            setRequestReadyState(xhr.readyState || 3)
          }

          loaded.current = progressEvent.loaded || 0
          total.current = progressEvent.total || 0

          if (contentType === 'json' && xhr?.responseText) {
            const currentRes = sliceData(xhr.responseText, progressEvent.loaded, loadedSize)
