  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '保存环境', location: '我的Notebook运行中行操作', type: '弹窗表单 + 制作镜像页', required: '否', description: '支持选择构建为标准镜像或保存为临时镜像；选择标准镜像后进入制作镜像配置页，返回或取消不生效，点击构建后才生成标准镜像；临时镜像仍在弹窗内直接保存。' },
      ...
      actions: [
        ...
        { name: '保存环境', entry: '运行中 Notebook 行操作', precondition: 'Notebook 状态为运行中', successFeedback: '选择临时镜像时弹窗确认后生成镜像记录；选择标准镜像时进入制作镜像页，点击构建后生成镜像记录', errorFeedback: '镜像名称为空时表单校验提示；制作页返回或取消不保存' },
      ...
      interactionNotes: [
        ...
        'Notebook 点击停止时先弹出确认弹窗，询问是否保存当前最新环境；选择“是”时在当前弹窗内直接展示标准镜像/临时镜像、包+依赖库、工作目录、镜像名称、镜像描述等保存环境同款字段。',
        ...
        '保存环境和停止前保存环境弹窗在原临时镜像基础上增加“构建为标准镜像”，默认选择标准镜像；两个镜像类型下方必须直接展示简短作用说明，不展示推荐标识或剩余次数。',
        '选择“构建为标准镜像”后进入制作镜像页，页面包含镜像名称、基础镜像、镜像描述、上传文件、Dockerfile 编辑区、已安装列表、取消和构建；返回或取消不生成镜像，只有点击构建后才保存为标准镜像，停止前保存场景在构建成功后再停止 Notebook。',
