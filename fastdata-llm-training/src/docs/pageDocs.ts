  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: 'SSH配置', location: '创建页/编辑页/详情页/列表', type: '用户名 + 密码', required: '否', description: 'SSH 配置下放到单个 Notebook 任务；创建/编辑页置于表单最下方，可选填用户名和密码；详情页固定展示，默认只读，右上角编辑/保存，复制使用轻量图标按钮。' },
      ...
      interactionNotes: [
        ...
        '详情页 SSH 配置卡片中用户名和密码默认只读展示，复制操作以不展示文字的轻量图标按钮放在对应输入框右侧且整行占满内容区；卡片右上角提供“编辑”操作，点击后切换为“保存”并允许修改，保存后再回到只读态。',
