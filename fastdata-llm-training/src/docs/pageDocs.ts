...
const globalDesignDoc = createDoc({
  ...
  userChanges: ['根据用户需求，需求评审和需求文档均新增全局模块，用于记录不归属单一页面或模块的需求。', '根据用户最新要求，在 V1.14 全局需求中补充任务创建页必填项自动定位体验优化。', '根据用户最新要求，需求评审中心的模块和页面排序改为全局需求优先，其余按菜单顺序展示。', '根据用户最新要求，全局需求补充 zip 压缩包上传解析规则：自动兼容压缩包内多一层同名文件夹的情况。', '根据用户最新要求，需求文档新增全屏评审展示模式，用于产品评审时更清晰展示需求内容。'],
...
- 所有支持 zip 的数据上传入口使用同一套路径归一化规则。`,
    },
    {
      ...
      content: `# V1.14 全局需求评审：需求文档全屏展示

## 1. 背景

产品进行需求评审时，右侧需求文档侧板宽度较窄，长需求内容展示空间不足，不利于会议中快速讲解、逐条确认和补充修改。

## 2. 目标

在保留现有右侧需求文档侧板、需求评审中心、版本管理和自动保存能力的基础上，新增全屏评审展示模式，让需求内容可以在更大的阅读区域中展示和编辑。

## 3. 交互规则

- 需求文档侧板顶部新增“全屏展示”按钮，与复制、关闭操作同层级。
- 点击“全屏展示”后，当前需求文档进入全屏评审模式，保留当前页面、当前模块、当前版本和当前 Tab。
- 全屏模式下按钮切换为“退出全屏”，点击后回到右侧侧板，不关闭需求文档。
- 全屏模式下仍允许新增、编辑、删除、拖拽排序需求和版本，并保持自动保存与变更记录。
- 需求评审中心点击页面需求时仍按现有方式打开页面与右侧侧板，不默认强制全屏。
- 支持通过 URL 参数 \`docMode=fullscreen\` 直接打开全屏评审模式，便于后续分享或评审入口扩展。

## 4. 展示要求

- 全屏态应覆盖主要页面区域，内容宽度明显大于侧板，适合会议投屏阅读。
- 文档内容区需要提升字号、行高和输入框高度，长文本阅读更轻松。
- 顶部仍展示页面名称、所属模块、当前页面/全局模块切换、复制、全屏切换和关闭入口。
- 窄屏下全屏模式应自适应宽度，操作区允许换行，不能横向溢出。

## 5. 验收标准

- 任意页面打开需求文档后，可以一键进入全屏展示。
- 全屏下编辑需求并退出全屏后，内容不丢失，侧板仍保持打开。
- 使用 \`?docOpen=1&docVersion=V1.14&docMode=fullscreen\` 访问页面时，自动打开对应版本并进入全屏。
- 需求评审中心原有列表、筛选、跳转行为不受影响。`,
    },
  ],
})
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 数据管理：新增文件管理

## 1. 需求说明

大模型数据管理需要补充生产环境已有的“文件管理”菜单，用于管理项目下通用文件夹，并作为后续 Notebook 等研发入口的数据来源补充。

## 2. 功能范围

- 文件管理入口位于数据服务 > 数据管理，与训练数据管理、测试数据管理、推理结果集同级。
- 页面路径使用生产环境口径 \`/file-management\`。
- 页面顶部展示标题“文件管理”和说明“管理各类的文件，支持多种格式上传、下载。”。
- 搜索区在“文件夹名”字段下支持按文件夹名称检索，提供刷新和重置按钮。
- 提供创建文件夹入口，创建时填写文件夹名称和文件夹描述。
- 列表字段为文件夹名称、文件夹描述、创建人、创建时间、操作。
- 文件夹名称前不展示额外小图标。
- 操作列提供文件管理和删除，删除需要二次确认。
- 点击文件管理进入详情页，详情页包含基本信息与文件信息。
- 基本信息展示文件夹名称、文件夹描述、创建人、创建时间。
- 文件信息展示文件名称、文件大小、文件类型和操作，支持上传文件、下载文件和删除文件。
- 文件信息表支持多选；勾选文件后展示批量下载和批量删除，批量删除需要二次确认。
- 文件夹列表支持横向滚动，保证窄屏下可以访问右侧字段和操作列。

## 3. 模块盘点

- 本轮明确接入：大模型在线 Notebook 创建页的数据选择需要新增“文件管理”数据类型。
- 本轮暂不接入：训练数据管理、测试数据管理、数据标注、数据清洗、推理结果集仍以数据集版本为选择对象，不直接选择文件管理，以免绕开已发布版本、数据用途和数据格式规则。
- 机器学习侧暂不新增文件管理菜单，保持机器学习数据管理独立，后续如生产环境补充同类入口再单独评估。

## 4. 验收标准

- 数据服务 > 数据管理下能看到文件管理菜单，点击进入文件管理页。
- 页面结构、标题、说明、搜索框、刷新/重置/创建文件夹按钮和表格列与生产环境保持一致。
- 创建文件夹后列表立即展示新文件夹，并记录创建人和创建时间。
- 列表点击文件管理可进入详情页，详情页布局与用户截图一致。
...
- 大模型在线 Notebook 与机器学习在线 Notebook 行为一致。`,
        },
        {
          ...
          content: `# V1.14 在线 Notebook：数据选择增加文件管理

## 1. 需求说明

大模型在线 Notebook 创建页的数据选择需要补充“文件管理”数据类型，让用户可以从数据管理下的文件管理中选择项目文件作为 Notebook 输入数据。

## 2. 交互规则

- 创建页“数据集”字段保留输入框/下拉回显和“选择”按钮。
- 点击“选择”后打开数据选择弹窗。
- 弹窗左侧数据类型包含训练数据集、验证数据集、测试数据集、文件管理。
- 选择文件管理后，右侧列表展示文件路径、格式、文件大小、更新时间和选择操作。
- 选择文件后，回填格式为“文件管理/文件夹/文件名”，并在 Notebook 详情页按数据集字段展示。

## 3. 边界说明

- 本轮只要求大模型在线 Notebook 创建页接入文件管理。
- 机器学习在线 Notebook 仍使用机器学习数据管理作为数据来源，暂不混入大模型文件管理。
- 数据标注、数据清洗、推理结果集仍按已发布数据集版本选择，不直接选择文件管理，避免绕开数据用途、格式和发布态规则。

## 4. 验收标准

- 大模型在线 Notebook 创建页点击数据集“选择”后可以看到文件管理数据类型。
- 切换到文件管理后可选择文件并正确回填。
- 训练/验证/测试数据集选择不受影响。`,
        },
      ],
...
- 页面文档与需求评审中心能检索到“大模型训练增加GRPO训练方法”。`,
        },
        {
          ...
          content: `# V1.14 GRPO训练：训练参数改为模板方式

## 1. 需求说明

GRPO 创建页不再让用户从固定默认参数开始填写，而是通过模板管理快速带出微调类型和训练参数。

## 2. 模板范围

- 模板只包含微调类型和训练参数。
- 支持全参微调模板和 LoRA 微调模板。
- 模板包含基础参数、高级配置、数据处理配置、评估配置、保存配置、监控配置。
- LoRA 微调模板额外包含 LoRA Rank、LoRA 目标模块、LoRA Alpha、LoRA Dropout。
- 模板不包含 RFT 算法、奖励规则、DeepSpeed 训练加速、模型来源、模型选择、数据集、资源规格、定时启动、任务名称和任务版本。

## 3. 创建页交互

- 当训练方法选择 RFT 且算法选择 GRPO 时，展示模板管理字段。
- 用户必须选择一个启用模板后才能创建 GRPO 训练任务。
- 选择配置后，系统展示整段 YAML 参数模板内容。
- 用户可直接编辑当前任务的参数模板内容，本次调整只影响当前训练任务，不回写系统配置。
- 奖励规则和训练加速配置继续保留在原位置，由用户按原交互配置。
- 奖励规则选择自定义代码时，参考模板通过 .py 文件下载提供，不在页面内直接展示模板代码。
- 下载模板只作为编写 reward_func 的起点；用户修改后仍需通过上传入口上传单个 .py 文件。
- GRPO 创建页不再展示基础参数、高级配置、数据处理配置、评估配置、保存配置、监控配置等原参数 Tabs。
- 选择 GRPO 后，显卡资源配置拆分为 Hand、Work、Submit 三个 Tab，三个阶段都需要设置。
- Hand 和 Work 配置显卡、CPU、内存；Submit 只配置 CPU 和内存。
- 版本详情页的显卡资源配置按相同三阶段展示。
- 旧版本如果缺少三阶段资源配置，详情页仍展示三阶段，并使用原单组资源兜底。
...
- 应用配置后直接展示可编辑的参数模板内容。
- 修改本次任务中的参数模板内容后，不影响系统配置中的配置模板。
- 自定义奖励函数参考模板可下载为 Python 文件，页面不直接展开代码块。
- 下载模板后上传自定义奖励函数仍校验单个 .py 文件与 reward_func 函数约束。
- 版本详情的参数配置直接展示本次训练保存的 YAML 模板内容。
- GRPO 版本详情的显卡资源配置展示 Hand、Work、Submit 三个阶段，不能回退到旧单组显卡资源表格。`,
        },
      ],
    }),
  },
...
- 权限配置包含“操作权限”和“可操作资源范围”两个 Tab，二者共同参与业务操作拦截。
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 系统配置：新增GRPO训练参数配置

## 1. 需求说明

系统配置需要新增 GRPO训练参数配置，让平台管理员把团队常用的 GRPO 微调类型和训练参数组合沉淀下来。算法工程师创建 GRPO 训练任务时先选择一套配置，再按当前任务需要微调参数，避免每次从空白默认值开始填写。

## 2. 模板范围

- 模板只管理微调类型和训练参数。
- 支持全参微调和 LoRA 微调。
- 支持基础参数、高级配置、数据处理配置、评估配置、保存配置、监控配置。
- LoRA 模板额外支持 LoRA Rank、LoRA 目标模块、LoRA Alpha、LoRA Dropout。
- 不配置 RFT 算法、奖励规则、DeepSpeed、模型、数据集、资源规格、任务名称、任务版本和定时启动。

## 3. 管理能力

- 支持新增、编辑、复制、删除、启用和停用配置。
- 新增或编辑时只维护模板名称、模板描述、启用状态和一整段参数模板内容。
- 参数模板内容采用 YAML 结构，根字段只允许 fineTuneType 和 params，params 只允许当前产品已有训练参数字段。
...
- 停用配置不出现在创建页可选列表。
- YAML 输入奖励规则、DeepSpeed、模型、数据或资源字段时需要校验失败。`,
        },
      ],
    }),
  },
