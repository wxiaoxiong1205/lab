...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '模板管理', location: '创建表单/训练配置', type: '选择框 + 可编辑 YAML 模板', required: 'GRPO时是', description: '用于选择一套 GRPO 训练参数模板，选择后直接展示 YAML 模板内容，用户可在本次任务内继续编辑。' },
        { name: '自定义奖励函数参考模板', location: '创建表单/训练配置/奖励规则配置', type: '文件下载', required: '否', description: '选择自定义代码奖励规则时，参考模板以 Python 文件下载形式提供，不在页面内直接展示代码。' },
      ...
      actions: [
        ...
        { name: '下载自定义奖励函数模板', entry: '创建页/奖励规则配置/自定义代码', precondition: '奖励规则类型选择自定义代码', successFeedback: '下载 .py 参考模板文件', errorFeedback: '浏览器下载失败时保持当前配置不变' },
      ],
      states: [
      ...
      defaultRequirements: [
        ...
        },
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              'GRPO 创建页不再让用户从固定默认参数开始填写，而是通过模板管理快速带出微调类型和训练参数。用户选择启用模板后，可在本次任务内继续编辑整段 YAML 参数模板。',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      module: '系统管理',
      status: '已基于需求演进',
      goal: '管理属性配置、标签配置、GRPO训练参数配置和文档中心 Agent 助手服务，并把平台级配置收敛在系统管理中。',
      audience: '平台管理员',
      problem: '让平台管理员既能维护基础配置，也能为 GRPO 创建流程配置可复用训练参数模板，并为文档中心配置唯一生效的全局 RAG 问答服务。',
      structure: ['属性配置 Tab', '标签配置 Tab', '模板管理 Tab', 'GRPO训练参数左侧菜单', '标签配置左侧分组', 'Agent助手 Tab', '文档中心单目录', 'Agent服务单配置卡', 'Agent 服务配置弹窗', '标签值弹窗'],
      fields: [
        ...
        { name: 'GRPO训练参数配置', location: '模板管理 Tab', type: '参数配置列表 + YAML 模板弹窗', required: '否', description: '把 GRPO 常用微调类型和训练参数沉淀为可复用配置；新增或编辑时只填写模板名称、描述，并直接维护一整段 YAML 参数模板内容。' },
      ...
      actions: [
        ...
        { name: '维护GRPO训练参数配置', entry: '模板管理 Tab', precondition: '进入系统配置', successFeedback: '配置保存后可在 GRPO 创建页选择', errorFeedback: '模板字段越界或基础信息校验失败时提示' },
      ...
      states: [
        ...
        { name: 'GRPO配置启用', meaning: '配置可被 GRPO 创建页选择', presentation: '配置列表启用开关为开启', availableActions: '编辑、复制、删除、停用' },
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            title: 'V1.14 系统配置：新增GRPO训练参数配置',
            description: [
              '系统配置新增 GRPO 训练参数配置，让平台管理员把团队常用的 GRPO 微调类型和训练参数组合沉淀为模板。算法工程师创建 GRPO 训练任务时先选择模板，再按当前任务需要微调参数，避免每次从空白默认值开始填写。',
              '模板只管理微调类型和训练参数，支持全参微调和 LoRA 微调；不配置 RFT 算法、奖励规则、DeepSpeed、模型、数据集、资源规格、任务名称、任务版本或定时启动。',
            ],
            modules: [
              '系统配置 > 模板管理 Tab > GRPO训练参数。',
              '大模型训练创建页 RFT-GRPO 参数模板选择。',
            ],
            acceptance: [
              '系统配置页可进入模板管理 Tab，并在左侧选择 GRPO训练参数。',
              '支持新增、编辑、复制、删除、启用和停用配置。',
              '新增或编辑时只维护模板名称、模板描述、启用状态和整段 YAML 参数模板内容。',
            ...
            interactions: [
              'LoRA 模板额外支持 LoRA Rank、LoRA 目标模块、LoRA Alpha、LoRA Dropout。',
            ],
          }),
        },
      ],
