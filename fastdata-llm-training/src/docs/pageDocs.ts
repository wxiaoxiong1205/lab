  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '最新任务', location: '任务分组卡片', type: '任务列表', required: '是', description: '按定时待启动、启动中、排队中、运行中、失败分组展示任务名称、创建人、创建时间和任务模块；每个状态模块使用固定高度并预留完整 4 条任务展示空间，翻页前后模块高度保持不变；超过 4 条时在分组底部居中展示上一页、数字页码和下一页。' },
      ],
      actions: [
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            acceptance: [
              ...
              '当前项目算力与对应集群算力展示在同一模块内，分母均为绑定集群总量，项目分子受任务范围影响，集群分子不受任务范围影响。',
              '最新任务按定时待启动、启动中、排队中、运行中、失败分组展示，至少包含任务名称、创建人、创建时间和任务模块。',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: [
        ...
        '本轮盘点结果：文件管理作为通用文件来源，优先接入大模型在线 Notebook 创建页的数据选择；训练/测试数据管理、数据标注、数据清洗、推理结果集仍按数据集版本链路选择，暂不直接加入文件管理，避免破坏已发布版本和数据用途规则。',
      ],
      productionComparison: ['本轮按用户提供的生产环境链接进行可见页复刻；生产当前页面包含“文件夹名”搜索输入、刷新、重置、创建文件夹，以及文件夹名称、文件夹描述、创建人、创建时间、操作列；文件夹详情页按用户补充截图实现。'],
      userChanges: ['根据用户新增需求，大模型数据管理补充文件管理菜单与页面，并同步盘点其它模块是否接入文件管理；在线 Notebook 创建页作为本轮明确下游接入文件管理数据类型。', '根据本轮批注，文件夹列表删除名称前小图标，补齐文件夹描述列，操作列改为文件管理和删除，并补充文件夹详情页。', '根据本轮反馈，文件详情表勾选文件后展示批量下载和批量删除按钮，外层文件夹列表支持横向滚动。'],
      syncMissingDefaultRequirements: true,
      defaultRequirements: [
        {
          version: 'V1.14',
          ...
          content: formatRequirementDoc({
            ...
            description: [
              '大模型数据管理补充“文件管理”菜单，用于管理项目下通用文件夹和文件，并作为在线 Notebook 等研发入口的数据来源补充。',
            ...
            interactions: [
              ...
              '本轮仅明确接入大模型在线 Notebook；训练/测试数据管理、标注、清洗、推理结果集仍按数据集版本选择，不直接选择文件管理。',
            ],
          }),
        },
      ],
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '数据集', location: '创建页/详情页', type: '下拉选择 + 选择弹窗', required: '否', description: '数据集与模型拆分为两个独立字段；大模型在线 Notebook 的选择弹窗增加文件管理数据类型，可从文件管理中选择项目文件；机器学习在线Notebook仅可选择机器学习-数据管理中的数据集。' },
      ...
      interactionNotes: [
        ...
        '大模型在线 Notebook 创建页的数据选择弹窗补充“文件管理”数据类型，与训练数据集、验证数据集、测试数据集并列；选择文件管理后展示文件路径、格式、文件大小和更新时间，并回填到数据集字段。',
      ],
      ...
      userChanges: [
        ...
        '根据本轮新增需求，大模型在线 Notebook 创建页的数据选择中增加文件管理数据类型，可从文件管理选择项目文件作为 Notebook 输入数据来源。',
      ],
      syncMissingDefaultRequirements: true,
      defaultRequirements: [
        {
        ...
        },
        {
          ...
          content: formatRequirementDoc({
            title: 'V1.14 在线 Notebook：数据选择增加文件管理',
            description: [
              '大模型在线 Notebook 创建页的数据选择补充“文件管理”数据类型，允许用户从数据管理下的文件管理中选择项目文件作为 Notebook 输入数据。',
              '该能力只补充 Notebook 研发场景的数据来源，不改变训练/验证/测试数据集原有选择逻辑，也不绕开数据集版本发布态规则。',
            ],
            modules: [
              '大模型在线 Notebook 创建页。',
              '数据选择弹窗。',
            ...
            acceptance: [
              '大模型在线 Notebook 创建页点击数据集“选择”后，可以在弹窗左侧看到文件管理数据类型。',
              '切换到文件管理后，右侧列表展示文件路径、格式、文件大小、更新时间和选择操作。',
              '选择文件后，创建页按“文件管理/文件夹/文件名”格式回填，并在 Notebook 详情页按数据集字段展示。',
              '训练数据集、验证数据集、测试数据集的原选择能力不受影响。',
            ],
            interactions: [
              '本轮仅大模型在线 Notebook 接入文件管理；机器学习在线 Notebook 仍使用机器学习数据管理，数据标注、数据清洗、推理结果集仍按已发布数据集版本选择。',
            ],
          }),
        },
      ],
