        ...
        },
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              ...
              '发布状态只属于具体版本，不用一个数据集外层状态代表所有版本。未发布版本可继续删除单条数据；已发布版本明细锁定。',
            ],
            modules: [
              '训练数据管理列表、详情页和版本侧栏。',
              '验证数据管理列表、详情页和版本侧栏。',
            ...
            interactions: [
              '详情页版本状态只展示处理失败、未发布、已发布三类，避免出现历史版本或已归档等额外状态。',
            ],
          }),
        },
        {
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            ...
            description: [
              ...
              '发布状态只属于具体版本，测试数据集外层列表不展示单一发布状态列，避免用一个状态代表所有版本。',
            ],
            modules: [
              '测试数据管理列表、详情页和版本侧栏。',
            ...
            interactions: [
              '详情页版本状态标签只展示处理失败、未发布、已发布三类，不展示历史版本或已归档。',
            ],
          }),
        },
        {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '标注模板', location: '创建页/列表/详情', type: '联动单选', required: '是', description: '根据标注类型提供对应模板；图片-图像分割仅提供实例分割和语义分割，回形/带孔标注归属实例分割。' },
        { name: '数据模板', location: '创建页上传数据区域', type: '模板下载', required: '否', description: '按数据类型、标注类型和标注模板动态展示当前上传模板；文本类提供 CSV/JSONL，图片类提供 ZIP。实例分割 ZIP 内同时包含普通实例分割和回形/带孔实例分割两个文件夹。' },
      ...
      actions: [
        ...
        { name: '下载数据模板', entry: '创建页上传数据区域的数据模板卡片', precondition: '已选择数据类型、标注类型和标注模板', successFeedback: '下载当前模板文件；实例分割模板下载 ZIP，内含普通实例分割和回形/带孔实例分割示例结构', errorFeedback: '无匹配模板时保持默认实例分割模板' },
      ...
      interactionNotes: [
        ...
        '图像分割模板拆成两类：实例分割和语义分割；回形/带孔标注不作为单独模板暴露，归属实例分割，支持外圈保留、内圈孔洞剔除，并在数据详情中展示 Mask 摘要。',
        '创建页文件上传区域新增数据模板卡片，会随标注类型和标注模板联动切换；图片类模板下载入口按生产环境文案展示为“ZIP 格式示例”，用户提供真实模板文件前，先以内置可下载模板覆盖文本分类、实体识别、图像分类、物体检测、语义分割和实例分割。',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '回形/带孔实例分割', location: '图像分割标注工作台', type: 'Mask 工作台', required: '否', description: '归属图片-图像分割-实例分割；通过数据集/任务类型进入专用工作台，RLE 仅作为内部存储和导出表示。' },
      ],
      actions: [
