...
const globalDesignDoc = createDoc({
  ...
  userChanges: ['根据用户需求，需求评审和需求文档均新增全局模块，用于记录不归属单一页面或模块的需求。', '根据用户最新要求，在 V1.14 全局需求中补充任务创建页必填项自动定位体验优化。', '根据用户最新要求，需求评审中心的模块和页面排序改为全局需求优先，其余按菜单顺序展示。'],
