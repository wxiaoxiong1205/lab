...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '训练加速配置', location: '创建表单/训练配置', type: '单选枚举', required: '否', description: '基于 DeepSpeed ZeRO 显存优化策略，可选择不开启、ZeRO-0、ZeRO-2 或 ZeRO-3；每个选项提供悬浮说明，帮助用户理解显存节省、通信开销和适用场景。' },
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', '旧结构：基础模型创建页按固定 Qwen 系列筛选模型版本并直铺在创建页；新结构：基础模型选择改为弹窗，弹窗左侧先选模型提供商，右侧展示该提供商下匹配当前训练类型的模型。', '本期以 Qwen 作为已适配模型示例，已下载模型可选，未下载模型展示“未下载，请联系管理员”；模型仓库新增的其它提供商模型也允许选择，但标记为“未适配”。', '弹窗标题行不展示“已适配/未适配”状态图例，适配状态仅保留在具体模型卡片上。', '训练配置新增 DeepSpeed 训练加速配置，默认不开启；可选择 ZeRO-0、ZeRO-2、ZeRO-3，对应仓库 examples/deepspeed/ds_z0_config.json、ds_z2_config.json、ds_z3_config.json。', 'ZeRO-0/2/3 选项必须提供悬浮说明：ZeRO-0 是普通 DDP 基线，ZeRO-2 切分优化器状态和梯度，ZeRO-3 切分优化器状态、梯度和参数。', '版本详情的显卡资源配置不再拆成显卡类型、型号、内存、张数，而是与创建页字段保持一致：显卡类型及型号、显卡卡数配置、CPU请求、CPU限制、内存请求、内存限制。', '训练任务详情页的任务版本表格设置明确列宽与横向滚动，小屏下可右滑查看创建时间后的操作列。', '创建图像理解训练任务时，训练数据集选择弹窗提供 SFT、DPO、RFT 场景下可演示的图像理解训练数据。', '根据本轮批注，图像理解 SFT 数据集选择弹窗的格式筛选不展示 PROMPT_RESPONSE，只展示 ROLE_BASED。', '训练任务从 mockTasks 直读改为共享本地 store；列表行内编辑名称和任务级描述后，任务详情、版本详情、创建/编辑版本页和资源引用提示读取同一任务对象。'],
      productionComparison: ['本轮未参考生产环境，按用户新增需求在当前仓库基线上迭代。'],
      userChanges: ['根据本轮需求，大模型训练创建页的基础模型选择改为展示模型仓库所有匹配训练类型的模型；Qwen 作为本期已适配示例，已下载可选，未下载提示联系管理员；其它提供商模型也可选择并标记未适配。', '根据本轮调整，基础模型选择改成弹窗：先选模型提供商，再选具体模型，避免在创建页直铺大量模型。', '根据本轮需求，大模型训练版本详情中的显卡资源配置字段与创建页所选字段保持一致，并展示 CPU/内存请求与限制。', '根据用户截图反馈，修复训练任务详情小屏下任务版本表格无法右滑、右侧操作列不可见的问题。', '根据本轮演示需求，补充创建图像理解训练任务时可选择的多模态训练数据集。', '根据本轮批注，图像理解 SFT 数据集格式收敛为 ROLE_BASED，避免在创建训练任务时出现 PROMPT_RESPONSE。', '根据本轮全局任务需求，大模型训练任务补任务级描述并支持名称/描述行内编辑，版本级 taskDescription 保持原语义。', '根据本轮上传的 DeepSpeed 配置文档，创建页新增训练加速配置，可选不开启、ZeRO-0、ZeRO-2、ZeRO-3，并为各策略提供悬浮注释。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['在线推理服务与大模型部署是两类不同服务，不再共用同一份设计文档。', '新增 /service/inference/external/create 路由，用于从系统配置 Agent助手中跳转创建对话模型服务；创建或取消后按 redirect 参数返回来源页。', '在线推理服务列表名称列和描述列统一接入字段级行内编辑，编辑入口仅在鼠标悬浮对应字段时展示，点击后当前字段变为输入框。'],
