...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['旧结构：文档中心左侧只有产品操作手册。', '新结构：文档中心左侧升级为多文档目录，产品操作手册和开发指南并列。', '开发指南路由为 /docs/developer-guide，/docs 默认仍进入产品操作手册。', '根据本轮批注，顶栏个人下拉不再提供身份切换入口，仅保留 API访问密钥、个人中心和设置。'],
      productionComparison: ['当前页以用户第二阶段新增需求为基线，不参考生产环境。'],
      userChanges: ['根据用户本轮需求，在文档中心新增开发指南文档，用于说明开放平台 API、认证方式、密钥创建、请求示例和错误码。', '根据浏览器批注，移除右上角个人下拉中的身份切换分组和用户列表。'],
      recentChanges: [
        {
          date: '2026-05-12',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '结果集名称', location: '列表', type: '文本', required: '是', description: '推理结果记录名称，支持在字段旁悬浮显示编辑入口并行内编辑。' },
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['创建页需要使用独立路由。', '左侧为算子选择，右侧为流程配置空态或已选算子列表。', '流程配置区不再展示清洗后数据集、流程结果预期和流程摘要卡片。', '列表支持按任务名称搜索和按清洗状态筛选。', '列表分页与生产环境一致默认 20 条/页，行操作展示启动、编辑、删除和更多入口。', '清洗任务详情弹窗拆为清洗详情和清洗日志两个页签；清洗详情按基本信息、清洗结果、算子标签和清洗前后对照表展示。', '当前页以用户迭代需求为基线；生命周期任务支持在列表名称列与描述列分别行内编辑，编辑入口悬浮显示且不受任务状态影响，详情和日志读取更新后的任务对象。'],
      productionComparison: ['本轮按用户要求直接参考生产环境；列表字段、筛选区、四步横向说明和分页规格对齐生产环境当前页面。'],
      userChanges: ['旧截图已删除并回退影响；数据清洗页改以生产环境当前页面为基线。', '根据本轮批注，创建页数据集选择对齐推理结果集选择弹窗，清洗字段改为按数据集格式回显单选，删除数据集名称、流程摘要、清洗后数据集和流程结果预期卡片，并补充定时执行时间配置。', '根据本轮批注，删除失败重试次数字段，清洗任务详情调整为基本信息与清洗结果结构，并保留清洗日志页签。', '根据本轮全局任务需求，清洗任务列表支持名称列与描述列分别行内编辑，编辑入口悬浮显示且不受任务状态影响，保存后详情、日志和资源引用提示同步刷新。'],
    }),
  },
  {
    match: pathname =>
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['大模型部署页已接入统一任务状态流转规则。', '当前页以用户迭代需求为基线；部署任务支持在列表名称列与描述列分别行内编辑，编辑入口悬浮显示且不受任务状态影响，详情读取更新后的任务对象。'],
