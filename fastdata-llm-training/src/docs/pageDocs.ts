...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '数据集名称', location: '列表/详情页/表单', type: '文本', required: '是', description: '唯一标识训练数据集；列表页与详情页支持行内编辑，详情页名称后常显编辑按钮。' },
        { name: '描述', location: '列表/详情页/表单', type: '长文本', required: '否', description: '训练数据集补充说明；列表页与详情页支持行内编辑，保存后同步数据集详情。' },
      ...
      actions: [
        ...
        { name: '编辑名称/描述', entry: '列表字段或详情页字段后的编辑按钮', precondition: '目标数据集存在', successFeedback: '保存后列表、详情和引用选项同步刷新', errorFeedback: '必填或长度校验失败时提示' },
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '测试数据集名称', location: '列表/详情页/表单', type: '文本', required: '是', description: '测试数据的主标识；列表页与详情页支持行内编辑，详情页名称后常显编辑按钮。' },
        { name: '描述', location: '列表/详情页/表单', type: '长文本', required: '否', description: '测试数据集补充说明；列表页与详情页支持行内编辑，保存后同步数据集详情。' },
      ...
      interactionNotes: ['页面头部不再展示面包屑，统一只保留标题、返回按钮和必要操作。', '去掉面包屑后，列表页头部采用克制的标题 + 说明结构，不新增统计卡；创建页、详情页和新增版本页统一改为“返回 + 标题 + 说明/操作”的紧凑结构。', '版本信息需要与列表信息联动展示。', '列表表格统一使用普通横向滚动，不再使用固定操作列，避免内容与操作区重叠。', '列表名称与描述进入编辑态后，取消按钮在前、确认按钮在后；详情页名称和描述字段后常显编辑按钮。', '详情页采用左侧版本切换、右侧信息与明细展示的结构。', '测试数据创建和新增版本的文件上传需展示进度，上传中可取消，失败或取消后可继续上传并保留断点进度；文件大小不设前端限制。', '根据用户最新要求，测试数据管理的数据用途从“文本生成/图像理解 + SFT/DPO/RFT 二级选择”收敛为单级枚举；创建页、列表筛选、列表列、详情页和新增版本页均只展示文本生成/图像理解。', '历史 mock 数据中带 SFT、DPO、RFT 前缀的数据用途在测试数据页按文本生成/图像理解归一展示。'],
      productionComparison: ['当前页以用户迭代需求为基线，本轮不参考生产环境。'],
      userChanges: ['根据用户最新反馈，测试数据管理创建页的数据用途只保留文本生成和图像理解，不再提供二级选择；列表和详情页的数据用途字段同步改为单级展示。', '根据本轮需求，被推理、标注、清洗或训练任务引用的测试数据集暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。', '根据本轮需求，测试数据详情页支持删除当前版本内单条数据，删除前二次确认并明确不可恢复。', '根据本轮需求，数据管理上传文件大小放开前端限制。', '根据本轮需求，测试数据列表与详情页支持编辑名称和描述，保存后走数据服务 store/API 统一同步；详情页改名后自动替换当前路由。'],
    }),
  },
  {
    match: pathname =>
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '结果集名称', location: '列表/详情页', type: '文本', required: '是', description: '推理结果记录名称，支持在列表行内编辑；详情页名称后常显编辑按钮。' },
