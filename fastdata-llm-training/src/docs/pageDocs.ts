...
const globalDesignDoc = createDoc({
  ...
  interactionNotes: ['当前页以用户迭代需求为基线。', '全局模块用于承接不单独属于某个业务模块的需求，例如全局壳层、权限规则、需求文档体系、跨模块交互和统一校验体验。', '全局需求不跳转到独立业务页面，需求评审中点击全局需求时保持当前页面并打开全局模块文档。', '需求评审中心按版本展示需求时，全局需求固定排在最前，业务模块按菜单顺序排列，同一模块内页面按菜单页面顺序排列。'],
  productionComparison: ['该模块为第二阶段协作需求新增，不来源于生产环境单页复刻。'],
  userChanges: ['根据用户需求，需求评审和需求文档均新增全局模块，用于记录不归属单一页面或模块的需求。', '根据用户最新要求，在 V1.14 全局需求中补充任务创建页必填项自动定位体验优化。', '根据用户最新要求，需求评审中心的模块和页面排序改为全局需求优先，其余按菜单顺序展示。', '根据用户最新要求，全局需求补充 zip 压缩包上传解析规则：自动兼容压缩包内多一层同名文件夹的情况。'],
  syncMissingDefaultRequirements: true,
  removedDefaultRequirementTitles: ['V1.14 全局需求：模型仓库与大模型训练基础模型联动'],
  defaultRequirements: [
    {
      version: 'V1.14',
...
- Notebook 和大模型训练等字段较多页面必须重点验证，不能只停留在顶部提示。`,
    },
    {
      ...
      content: `# V1.14 全局数据上传：兼容 zip 同名根目录

## 1. 背景

用户直接压缩文件夹上传 zip 时，压缩包内经常会多一层与压缩包同名的根目录，真实数据内容位于该目录下。如果系统只按压缩包首层解析，会误判数据结构或找不到有效内容。

## 2. 需求说明

所有涉及数据集上传且支持 zip 压缩包的入口，都需要兼容“压缩包根目录多一层同名文件夹”的情况。解析时应自动识别并展开同名根目录，将其下级内容作为真实数据内容处理。

## 3. 适用范围

- 大模型训练数据管理、测试数据管理新增与新增版本上传。
- 推理结果集导入上传。
- 机器学习数据管理新增与新增版本上传。
- 后续新增的其它 zip 数据上传入口。
...
- 当 zip 内所有有效文件都位于同一个根目录下，且根目录名称与 zip 文件名去除扩展名后一致时，系统自动剥离该根目录。
- 剥离后以下一级文件和目录作为数据集真实内容继续解析。
- 如果 zip 内存在多个根目录，或根目录名称与 zip 文件名不一致，则保持原路径结构，不做自动剥离。
- 自动剥离只影响解析路径，不修改用户原始上传文件。

## 5. 验收标准

- 上传 \`dataset.zip\`，内部结构为 \`dataset/images/...\`、\`dataset/labels/...\` 时，系统按 \`images/...\`、\`labels/...\` 解析。
- 上传本身没有同名根目录的 zip 时，解析结果不受影响。
- 上传包含多个根目录的 zip 时，系统不错误合并目录。
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 数据版本合并与来源展示

训练/验证数据管理需要支持同一数据集内多个版本合并。入口与新增版本放在详情页左侧版本操作区域，用户可选择至少 2 个处理完成版本，未发布但处理完成的版本也允许参与合并。

合并后生成一个新的最新版本，默认未发布，源版本不删除、不改写。合并样本全部保留，即使内容或样本名重复也不去重；每条合并后的明细可在后端记录来源关系，但前端数据详情表不展示来源字段。

不支持跨数据集、跨数据类型或跨数据格式合并。合并失败、版本不足或目标数据集不存在时需要给出明确提示。旧数据若只有最新版本记录，系统需补齐可持久化的版本记录后再合并，不能只在前端提示成功。`,
        },
        {
          version: 'V1.14',
...
下游训练、标注、清洗和推理数据选择按版本展开，已发布版本可选择，未发布版本保留展示但置灰且不可选择。若最新版本未发布，但同一数据集存在上一个已发布版本，下游仍可选择这个已发布版本。`,
        },
        {
          ...
          content: `# V1.14 GRPO训练：训练数据管理增加GRPO类型

## 1. 需求说明

训练/验证数据管理需要正式支持 GRPO 训练数据类型，用于承接大模型 RFT-GRPO 训练链路的数据准备。

## 2. 功能规则

- 数据用途二级枚举中保留 SFT、DPO、RFT-GRPO。
- 新建训练/验证数据集时，可选择文本生成/图像理解下的 RFT-GRPO 类型。
- RFT-GRPO 类型使用奖励/打分类数据格式，当前对应 Completion_Reward。
- 列表、详情、版本信息、筛选项和下游选择弹窗均需展示 RFT-GRPO 口径，不展示 PPO。
- RFT-GRPO 数据可被大模型训练、数据标注和数据清洗链路按已发布版本选择。

## 3. 验收标准

- 训练/验证数据管理创建页可以创建 RFT-GRPO 类型数据。
- 列表和详情可按 RFT-GRPO 展示与筛选。
- 已发布的 RFT-GRPO 数据可在下游 GRPO 相关链路中被选择。
...
下游选择按版本展开，已发布版本可选择，未发布版本保留展示但置灰且不可选择。若最新版本未发布，但同一测试数据集仍有历史已发布版本，下游继续允许选择该历史已发布版本。`,
        },
        {
          version: 'V1.14',
          createdAt: '2026-05-27T16:20:00+08:00',
          content: `# V1.14 测试数据版本合并与来源展示

测试数据管理需要支持同一测试数据集内多个版本合并。入口与新增版本放在详情页左侧版本操作区域，用户可选择至少 2 个处理完成版本，未发布但处理完成的版本也允许参与合并。

合并后生成一个新的最新版本，默认未发布，源版本不删除、不改写。合并样本全部保留，即使内容或样本名重复也不去重；每条合并后的明细可在后端记录来源关系，但前端数据详情表不展示来源字段。

未发布合并版本允许继续检查和调整，包括删除该版本里的单条数据；发布后当前版本明细锁定，不允许继续删除单条数据，可被推理、评估、标注和清洗引用。旧数据若只有最新版本记录，系统需补齐可持久化的版本记录后再合并，不能只在前端提示成功。`,
        },
      ],
    }),
  },
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 数据标注：数据选择用途筛选收敛

## 1. 需求说明

数据标注创建任务时，数据选择不再在创建表单中额外暴露数据集类型或多个用途字段。数据用途统一收敛到“选择数据集”弹窗内部，保持一个字段完成筛选。

## 2. 交互规则

- 创建页表单不展示独立的数据集类型字段。
- 点击“选择数据集”后打开数据集选择弹窗。
- 弹窗左侧仅保留一个“数据用途”字段，不拆成“数据场景”“训练方法”等多个字段。
- “数据用途”字段内部先选择文本生成或图像理解，再在当前分类下选择 SFT、DPO、RFT-GRPO。
- 筛选控件样式使用普通单选与缩进层级，需与数据类型、数据格式等上下筛选项保持一致。
- 选择数据集版本后，系统根据所选数据自动回填任务的数据类型与用途，不要求用户在创建表单二次选择。

## 3. 数据规则

- 数据标注仅展示已发布的数据集版本。
- 可选用途只包含文本生成/图像理解下的 SFT、DPO、RFT-GRPO。
- 选择 DPO 数据时，后续标注页按 Alpaca 或 Role-Based 数据格式展示对应字段。

## 4. 验收标准

- 数据标注创建页没有独立数据集类型字段。
- 数据选择弹窗左侧只有一个“数据用途”筛选字段。
- 切换文本生成/图像理解和 SFT/DPO/RFT-GRPO 后，右侧数据集列表与数据格式筛选同步刷新。
- 弹窗筛选样式与上下字段一致，不出现突兀的按钮组样式。`,
        },
        {
          version: 'V1.14',
          createdAt: '2026-05-28T18:22:00+08:00',
          content: `# V1.14 GRPO训练：数据标注增加GRPO选项

## 1. 需求说明

数据标注的数据选择弹窗需要支持 GRPO 数据选项，让用户可以针对 RFT-GRPO 训练数据创建标注任务。

## 2. 功能规则

- 数据选择弹窗的数据用途字段中，文本生成/图像理解下均展示 RFT-GRPO。
- 仅展示已发布的 RFT-GRPO 数据版本供标注任务选择。
- 选择 RFT-GRPO 数据后，标注任务自动继承数据用途和数据格式，不需要用户在创建表单二次选择。
- RFT-GRPO 与 SFT、DPO 使用同一套弹窗筛选交互：单一数据用途字段内先选文本生成/图像理解，再选具体方法。
- 弹窗与任务详情不展示 PPO 选项或 PPO 提示。

## 3. 验收标准

- 数据标注创建页打开数据选择弹窗后，可以在数据用途中选择 RFT-GRPO。
- 选择 RFT-GRPO 后，右侧列表只展示匹配的已发布数据版本。
- 创建标注任务后，任务输入数据与用途展示正确。
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 数据清洗：数据选择用途筛选对齐

## 1. 需求说明

数据清洗创建任务时，源数据集选择需要与数据标注保持同一套数据用途筛选交互，避免同类数据选择在不同模块中出现不同理解成本。

## 2. 交互规则

- 创建页通过“源数据集”只读输入框进入数据集选择弹窗。
- 弹窗左侧只保留一个“数据用途”字段，不额外拆出多个用途相关字段。
- “数据用途”字段内部先选择文本生成或图像理解，再在当前分类下选择 SFT、DPO、RFT-GRPO。
- 筛选控件采用普通单选和缩进层级样式，与数据类型、数据格式筛选保持一致。
- 切换数据用途后，右侧数据集列表和下方数据格式筛选需要同步刷新。

## 3. 数据规则

- 数据清洗仅展示已发布的数据集版本。
- 可选用途只包含文本生成/图像理解下的 SFT、DPO、RFT-GRPO。
- 选择不同数据格式后，清洗字段按格式自动回显，例如 DPO-Alpaca 展示 Instruction/Input/Chosen/Rejected，DPO-Role-Based 展示 Messages/Chosen/Rejected。

## 4. 验收标准

- 数据清洗创建页的数据选择弹窗与数据标注保持同款“单一数据用途字段”交互。
- 文本生成/图像理解与 SFT/DPO/RFT-GRPO 的组合筛选均可正常生效。
- 弹窗样式与上下筛选字段统一，不出现突兀的按钮组样式。
- 选中数据集后，清洗字段能按所选数据格式正确回显。`,
        },
        {
          version: 'V1.14',
          createdAt: '2026-05-28T18:24:00+08:00',
          content: `# V1.14 GRPO训练：数据清洗增加GRPO选项

## 1. 需求说明

数据清洗创建任务时，源数据集选择需要支持 GRPO 数据选项，使 RFT-GRPO 训练数据可以进入清洗链路。

## 2. 功能规则

- 源数据集选择弹窗的数据用途字段中，文本生成/图像理解下均展示 RFT-GRPO。
- 仅展示已发布的 RFT-GRPO 数据版本供清洗任务选择。
- 选择 RFT-GRPO 数据后，清洗字段按对应数据格式自动回显。
- RFT-GRPO 与 SFT、DPO 使用同一套单字段层级筛选交互。
- 弹窗与创建页不展示 PPO 选项或 PPO 提示。

## 3. 验收标准

- 数据清洗创建页打开源数据集选择弹窗后，可以在数据用途中选择 RFT-GRPO。
- 选择 RFT-GRPO 后，右侧列表和数据格式筛选同步刷新。
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 大模型 Notebook 广场内置案例

## 1. 需求说明

大模型在线 Notebook 的 Notebook 广场需要提供平台内置案例，帮助用户快速了解 Notebook 在大模型场景中的典型使用方式，并可直接复制到 Notebook 环境中运行。

## 2. 功能规则

- Notebook 广场需展示大模型方向的内置案例卡片。
- 内置案例详情页需采用图文长文档结构，支持案例说明、运行环境、数据准备、代码块和执行流程说明。
- 内置案例必须包含可运行的数据准备与代码内容，不能只提供静态文字说明。
- 广场卡片和详情页均需支持复制案例内容。
- 本需求只定义大模型 Notebook 广场需要具备内置案例能力，不在需求文档中展开具体案例内容。
...
- 案例内容具备可运行的数据准备和代码结构。`,
        },
        {
          ...
          content: `# V1.14 在线 Notebook：任务级 SSH 配置

## 1. 需求说明

Notebook 的 SSH 配置不再由项目管理统一设置，改为下放到每个 Notebook 任务。每个 Notebook 创建时可独立配置 SSH 用户名和密码，未配置的 Notebook 也可以在详情页后续补充。

## 2. 功能规则

- 大模型在线 Notebook 和机器学习在线 Notebook 均适用同一规则。
- 创建 Notebook 时在表单最下方展示 SSH 配置区域，字段包含用户名和密码，均为非必填项。
- 编辑 Notebook 时回填该 Notebook 已保存的 SSH 配置，并允许修改。
- Notebook 列表中的 SSH 配置列按单条 Notebook 判断，已填写用户名或密码则展示已配置，否则展示未配置。
- Notebook 详情页固定展示 SSH 配置卡片，不因创建时未填写而隐藏。
- 用户可在详情页补充、修改或清空当前 Notebook 的 SSH 配置，保存后列表状态与详情展示同步更新。
- 详情页用户名、密码默认只读；点击卡片右上角“编辑”后进入编辑态，按钮切换为“保存”，点击保存后生效并回到只读态。
- 详情页用户名、密码输入框右侧分别提供轻量复制图标按钮，不展示“复制”文字；复制读取当前输入框内容，未填写时提示用户先填写。
- SSH 配置属于 Notebook 任务自身配置，不再继承项目管理中的 SSH 配置。

## 3. 验收标准

- 创建 Notebook 页面最下方可以看到 SSH 配置区域，用户名和密码不填也能正常创建。
- 创建时填写 SSH 配置后，列表展示已配置，详情页可看到并通过右上角“编辑/保存”继续维护，且用户名和密码都支持通过输入框旁的轻量图标按钮复制。
- 创建时不填写 SSH 配置后，列表展示未配置，详情页仍展示 SSH 配置卡片并可保存。
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 大模型训练：基础模型与我的模型选择优化

## 1. 需求说明

大模型训练创建页的基础模型配置需要同时支持“模型仓库”和“我的模型”两类来源。

模型来源为模型仓库时，需要展示模型仓库中的全部模型，并根据模型系列、下载状态和已适配名单区分可选状态。

模型来源为我的模型时，需要与模型仓库保持一致的弹窗选择体验：创建页只展示选择入口和已选摘要；弹窗内支持按训练类型和训练方法筛选模型，并在模型卡片下选择具体版本。

## 2. 交互规则

### 2.1 模型仓库选项

- 基础模型选择器展示模型仓库全部模型，不因模型提供商或模型系列提前过滤隐藏。
- Qwen 系列模型进入已适配名单判断。
- Qwen 已下载模型展示“已适配”，允许选择。
- Qwen 未下载模型展示“已适配 + 未下载，请联系管理员”，不可选择。
- 非 Qwen 系列或不在已适配名单中的模型只展示“未适配”状态，允许选择，不额外展示其它提示。
- 未适配模型仍需展示在列表中，避免用户误以为模型不存在。

### 2.2 我的模型版本

- 模型来源切换为“我的模型”后，创建页字段名称为“我的模型版本”。
- “我的模型版本”不在创建页直铺卡片，统一使用弹窗选择。
- 弹窗左侧包含训练类型筛选：文本生成、图像理解。
- 弹窗左侧包含训练方法筛选：全部、SFT、DPO、RFT。
- 训练类型与训练方法组合过滤右侧模型列表。
- 右侧先展示模型卡片，再在模型卡片内选择具体版本。
- 选择版本后关闭弹窗，并在创建页展示已选模型名称、版本、训练类型和训练方法摘要。

## 3. 验收标准

- 大模型训练创建页的基础模型选择器可以看到模型仓库中的全部模型。
- Qwen 已下载模型可选择，并显示“已适配”。
- Qwen 未下载模型不可选择，并显示“已适配 + 未下载，请联系管理员”。
- 非适配名单模型可以选择，并显示“未适配”。
- 选择“我的模型”后，页面仅展示选择入口，不再直铺模型卡片。
- 打开“我的模型”弹窗后，可以按文本生成/图像理解和全部/SFT/DPO/RFT 组合筛选。
- 点击模型版本后，创建页回显所选模型版本摘要。`,
        },
        {
          version: 'V1.14',
          createdAt: '2026-05-28T18:26:00+08:00',
          content: `# V1.14 GRPO训练：大模型训练增加GRPO训练方法

## 1. 需求说明

大模型训练创建页需要支持 GRPO 训练方法。GRPO 作为当前开放的 RFT 算法，用于强化微调训练链路。

## 2. 功能规则

- 训练方法中保留 SFT、DPO、RFT。
- 当用户选择 RFT 时，RFT 算法只开放 GRPO。
- 选择 RFT-GRPO 后，训练数据集选择弹窗只展示与 GRPO 兼容的数据。
- GRPO 训练支持文本生成和图像理解两类训练类型。
- 我的模型选择、数据选择和任务详情展示统一使用 RFT/GRPO 口径，不展示 PPO。
- 创建成功后，训练任务、版本详情和模型产物中的训练方法应能追踪到 RFT-GRPO。

## 3. 验收标准

- 大模型训练创建页选择 RFT 后，仅可选择 GRPO 算法。
- 选择 GRPO 后，训练数据选择能筛出 RFT-GRPO 类型数据。
- 创建后的任务详情和版本信息能体现 RFT-GRPO 训练口径。
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 我的模型：版本表字段与关联任务展示优化

## 1. 需求说明

我的模型详情页需要以“模型资产 + 版本明细”的结构展示版本来源和训练上下文。

外层模型列表和详情基本信息不再展示统一的基础模型字段；基础模型、训练方法和来源上下文都下沉到版本维度展示。

## 2. 字段规则

- 外层列表删除基础模型字段，只展示模型名称、模型类型、版本数量和操作。
- 详情页基本信息删除基础模型字段，只展示模型名称、模型类型和模型来源。
- 模型版本表新增基础模型字段，展示“模型名 + 模型仓库/我的模型”来源标签。
- 模型版本表在基础模型右侧新增训练方法字段，展示 SFT、DPO 或 RFT。
- 原“版本来源”字段改为“关联任务”字段。

## 3. 关联任务展示规则

- 关联任务不重复展示“大模型训练”或“在线 Notebook”来源标签，因为详情页基本信息已展示模型来源。
- 来源为大模型训练时，第一行展示“任务名 - 版本”，第二行展示 Checkpoint。
- 来源为在线 Notebook 时，第一行展示任务名，第二行展示对应文件名。
- 任务名、版本、文件名和 Checkpoint 的字号层级需要统一，不出现同一行前后字重不一致的问题。

## 4. 操作规则

- 启动、终止、查看日志只对 LoRA 类型模型版本展示。
- 非 LoRA 类型模型版本只展示编辑和删除。
- 已完成版本的编辑按钮置灰，不隐藏。

## 5. 验收标准

- 模型详情页版本表可以看到关联任务、基础模型、训练方法、状态、创建人和创建时间。
...
机器学习数据标注的数据源按版本展示，已发布版本可选择，未发布版本保留展示但置灰且不可选择。若最新版本未发布，但同一数据集存在历史已发布版本，标注创建仍可选择这个历史已发布版本。`,
        },
        {
          version: 'V1.14',
          createdAt: '2026-05-27T16:20:00+08:00',
          content: `# V1.14 机器学习数据版本合并与来源展示

机器学习数据管理需要支持同一数据集内多个版本合并。入口与新增版本放在详情页左侧版本操作区域，用户可选择至少 2 个版本合并。

合并后生成一个新的最新版本，源版本不删除、不改写。合并样本全部保留，即使内容或样本名重复也不去重；后端记录来源关系，前端数据详情表不展示来源字段，样本重名处理属于后端保存逻辑。合并生成的未发布版本允许删除版本里的单条数据；发布后该版本明细锁定，不允许继续删除单条数据。

合并生成的版本在基本信息中将数据来源展示为“版本合并”，不展示具体来源关系。前台不展示样本名字段，不暴露后端用于保存和去重的内部标识。`,
        },
      ],
    }),
  },
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 机器学习模型部署：自定义部署

## 1. 需求说明

机器学习模型部署需要支持自定义部署方式，允许用户不依赖模型管理中的标准模型版本，直接基于自定义镜像、启动命令、端口、资源和服务配置创建机器学习在线服务。

## 2. 功能规则

- 创建部署时提供“标准部署”和“自定义部署”两种部署方式，用户选择自定义部署后进入自定义部署表单。
- 自定义部署的基础信息包含服务名称和服务描述。
- 自定义部署的环境信息只允许选择自定义镜像，不展示系统镜像入口，也不提供系统镜像/自定义镜像切换。
- 自定义部署需填写运行命令、端口号和部署实例数。
- 自定义部署需配置资源信息，包括 CPU 请求/限制、内存请求/限制、显卡类型和显卡数量。
- 自定义部署的服务配置使用 JSON 代码编辑框维护，提交时需校验 JSON 格式。
- 提交成功后生成自定义部署记录，列表展示部署方式、镜像或服务目标、资源规格、实例数、状态、创建人和创建时间。
- 自定义部署记录支持查看详情、编辑回填、启动/重新提交、终止和删除，操作规则沿用统一任务生命周期。

## 3. 验收标准

- 进入机器学习模型部署创建页，选择自定义部署后可看到自定义部署表单。
- 自定义部署环境信息中只出现自定义镜像选择，不出现系统镜像。
- 必填项缺失或服务配置 JSON 格式错误时不可提交，并提示对应错误。
- 创建成功后列表新增自定义部署记录，编辑时能回填原配置。
- 自定义部署记录的状态操作符合统一任务生命周期规则。`,
        },
      ],
      recentChanges: [
        {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['项目创建/编辑不再维护项目管理员字段；项目管理员仍是内置角色，但通过成员管理分配给项目用户。', '成员管理添加用户时必须选择项目角色，角色选项只能来自该用户在控制台已授权的角色。', '点击添加成员后立即写入项目成员列表，同时保留保存按钮用于确认整批成员调整。', '成员进入项目后，是否只能操作个人数据或可操作全部数据，由该项目角色的数据权限决定。', '项目管理不再维护 SSH 配置；SSH 配置已下放到每个 Notebook 任务创建页和详情页。', '项目表格使用普通横向滚动，不使用固定列，避免小屏下操作列覆盖中间字段。', '删除项目必须先进行二次确认。'],
      productionComparison: ['该页以用户第二阶段权限需求为基线，不再沿用项目创建页选择项目管理员的旧结构。'],
      userChanges: ['根据用户最新权限需求，项目创建/编辑删除项目管理员字段。', '成员管理增加项目角色选择，且只能从用户在控制台已授权角色中选择。', '成员列表角色字段展示当前项目角色，数据范围由权限配置页的数据权限 tab 决定。', '根据本轮批注，项目创建提示删除租户管理员相关描述，并修复添加成员点击后不立即生效的问题。', '根据本轮 Notebook SSH 配置调整，项目管理删除项目级 SSH 配置入口，SSH 改为在单个 Notebook 任务中维护。'],
    }),
  },
  {
      ...
      defaultRequirements: [
        {
          ...
          content: `# V1.14 模型仓库：放开模型提供商并删除支持能力
...
- 模型仓库放开模型提供商限制，支持来自不同来源或不同厂商的基础模型。
- 新增、编辑、列表和详情均不再展示“支持能力”字段。
- 模型是否可用于大模型训练，不在模型仓库页通过“支持能力”静态判断，而由大模型训练创建页的基础模型选择器根据适配名单和下载状态展示。
