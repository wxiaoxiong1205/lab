...
const pageDocs: PageDocEntry[] = [
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['登录后默认进入项目空间，而不是直接进入业务模块。', '点击项目卡片后才显示项目内具体功能菜单；大模型入口进入大模型业务默认页，机器学习入口进入机器学习业务默认页，任务概览由顶部同级入口进入。', '顶部主导航包含“任务概览、项目空间、系统管理”，任务概览与项目空间/系统管理同层。', '当前页以用户迭代需求为基线。根据本轮批注，站内消息相关开发内容暂不启用，顶部 Bell/通知中心入口从前端屏蔽。'],
      ...
      defaultRequirements: [
        {
          ...
          content: formatRequirementDoc({
            title: 'V1.14 任务概览需求',
            description: [
              '将原“大模型首页”抽离为顶层“任务概览”入口，与项目空间、系统管理同级。用户进入后默认进入自己有权限的最近项目，快速查看该项目下需要算力资源的任务状态、资源占用和近期风险任务。',
              '任务概览只做项目级算力任务看板，不替代大模型或机器学习工作区；用户仍通过顶部业务导航回到数据服务、模型训练、模型评估、模型服务或机器学习模块。',
              '统计对象仅包含需要配置算力资源的任务，不统计数据集、模型、标注资源、存储配置、权限配置等资源类实体。',
            ],
            modules: [
              '顶层导航：任务概览、项目空间、系统管理。',
              '任务概览页：项目选择、任务范围筛选、任务状态统计、算力资源、最新任务分组。',
              '大模型算力任务：训练、推理结果集、数据清洗、效果评估、模型部署、在线 Notebook 等。',
              '机器学习算力任务：机器学习模型部署及后续新增的机器学习算力型任务。',
            ],
            acceptance: [
              '进入任务概览后，默认展示当前用户有权限项目下的全部算力型任务；无可访问项目时展示空态。',
              '切换项目后，任务状态统计、算力资源和最新任务全部刷新为新项目数据。',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '保存环境', location: '我的Notebook运行中行操作', type: '弹窗表单', required: '否', description: '支持选择构建为标准镜像或保存为临时镜像；默认勾选且不可取消“包+依赖库”，可选工作目录并填写镜像名称、镜像描述。' },
        { name: '自定义镜像', location: '独立镜像管理页', type: '列表与添加弹窗', required: '否', description: '按生产环境形态展示镜像、描述、镜像类型、状态、任务来源、标签、创建人、创建时间与操作。' },
      ...
      interactionNotes: [
        ...
        '保存环境和停止前保存环境弹窗在原临时镜像基础上增加“构建为标准镜像”，默认选择标准镜像；两个镜像类型下方需展示简短作用说明，但不展示推荐标识或剩余次数；手动添加镜像时也需要选择镜像类型。',
