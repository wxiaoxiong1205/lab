...
const pageDocs: PageDocEntry[] = [
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['登录后默认进入项目空间，而不是直接进入业务模块。', '点击项目卡片后才显示项目内具体功能菜单。', '顶部主导航只保留“项目空间”和“系统管理”两个 Tab。', '当前页以用户迭代需求为基线。旧结构：右上角 Bell 仅展示静态红点数字；新结构：Bell 接入可交互消息中心，未读数来自通知 store，点击后以右侧抽屉展示 Lab 任务站内消息。'],
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['入口位于右上角个人按钮下拉菜单，菜单名称为 API访问密钥。', '访问密钥归属当前登录账号，不归属当前项目，进入该页时左侧不展示项目菜单，卡片标题区不再重复展示当前账号。', '页面功能描述改为说明该密钥用于访问开放平台 API，并在标题区操作位提供开放平台文档入口，点击跳转 /docs/developer-guide。', '每个账号仅允许创建一个访问密钥，密钥永久有效，不提供有效期、禁用、备注和名称字段。', 'Access Key ID 明文展示并可复制；Secret Access Key 中间部分星号脱敏展示，复制按钮复制完整值。', '当前页以用户第二阶段迭代需求为基线。', '本轮先使用 localStorage mock 数据，并通过 openPlatformApi 服务层保留未来接真实后端的边界。'],
      productionComparison: ['当前页以用户第二阶段新增需求为基线，不参考生产环境。'],
      userChanges: ['根据用户本轮需求，右上角个人按钮入口由开放平台 API 改为 API访问密钥。', '访问密钥调整为个人账号级资源，不跟项目走，开放平台路由不展示左侧项目菜单。', '密钥能力调整为每个账号只允许创建一个，永久有效，列表字段固定为 Access Key ID、Secret Access Key、创建时间和删除操作。', '根据浏览器批注，修改页面功能描述，新增开放平台文档跳转入口，删除卡片右侧当前账号展示，并支持 Access Key ID 与 Secret Access Key 复制，其中 Secret Access Key 中间脱敏。'],
      recentChanges: [
        {
          date: '2026-05-12',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['页面头部不再展示面包屑，统一只保留标题、返回按钮和必要操作。', '去掉面包屑后，列表页头部采用克制的标题 + 说明结构，不新增统计卡；创建页、详情页和新增版本页统一改为“返回 + 标题 + 说明/操作”的紧凑结构。', '版本信息需要与列表信息联动展示。', '列表表格统一使用普通横向滚动，不再使用固定操作列，避免内容与操作区重叠。', '详情页采用左侧版本切换、右侧信息与明细展示的结构。', '测试数据创建和新增版本的文件上传需展示进度，上传中可取消，失败或取消后可继续上传并保留断点进度；文件大小不设前端限制。', '根据用户最新要求，测试数据管理的数据用途从“文本生成/图像理解 + SFT/DPO/RFT 二级选择”收敛为单级枚举；创建页、列表筛选、列表列、详情页和新增版本页均只展示文本生成/图像理解。', '历史 mock 数据中带 SFT、DPO、RFT 前缀的数据用途在测试数据页按文本生成/图像理解归一展示。'],
      productionComparison: ['当前页以用户迭代需求为基线，本轮不参考生产环境。'],
      userChanges: ['根据用户最新反馈，测试数据管理创建页的数据用途只保留文本生成和图像理解，不再提供二级选择；列表和详情页的数据用途字段同步改为单级展示。', '根据本轮需求，被推理、标注、清洗或训练任务引用的测试数据集暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。', '根据本轮需求，测试数据详情页支持删除当前版本内单条数据，删除前二次确认并明确不可恢复。', '根据本轮需求，数据管理上传文件大小放开前端限制。'],
    }),
  },
  {
    match: pathname =>
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '结果集名称', location: '列表', type: '文本', required: '是', description: '推理结果记录名称，支持按统一生命周期规则行内编辑。' },
        { name: '结果集描述', location: '列表/创建页/详情页', type: '长文本', required: '否', description: '推理结果集任务级描述，最多 300 字。' },
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['旧截图已不作为设计依据；数据标注页以生产环境当前页面为基线进行收敛。', '根据本轮明确需求，补齐查看详情的逐条标注工作台、完成自动下一条、全量完成后提交、提交后不可编辑，以及在线推理服务标注配置。', '根据本轮要求，数据标注列表补充多条演示任务，并覆盖未开始、标注中、待审核、已完成、已提交、失败等状态；已完成详情切换为只读数据模式。', '根据本轮批注，图像标注详情改为图像理解结构，多人标注按项目管理员视角补齐任务总览、标注任务和审核任务。', '根据最新批注，删除多人标注页当前身份展示，补齐任务成员管理、删除二次确认、多人标注整页创建以及多人详情的标注/审核进度。', '根据本轮批注，多人标注创建者详情改为只读进度看板，按页展示多条数据的标注/审核进度；任务成员弹窗取消截止时间编辑和移除成员，改为固定截止时间与替换成员。', '根据本轮批注，替换成员弹窗改为单选且逐个替换；多人详情行级标注/审核进度改为文字状态，不再使用进度条。', '根据本轮反馈，标注任务和审核任务入口不再共用任务总览详情，分别进入标注操作工作台和审核操作工作台。', '根据最新反馈，审核任务详情改为一页一条数据，审核操作收敛为审核结果列，并补充未通过原因和完成审核后自动下一条。', '根据本轮澄清，数据标注删除指进入标注/审核页后删除单条数据；只要任务未提交，无论该条是否已标注均可删除，已提交后不允许删除。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '清洗任务名称', location: '表单/列表', type: '文本', required: '是', description: '清洗任务标识，支持按统一生命周期规则行内编辑。' },
      ...
      interactionNotes: ['创建页需要使用独立路由。', '左侧为算子选择，右侧为流程配置空态或已选算子列表。', '流程配置区不再展示清洗后数据集、流程结果预期和流程摘要卡片。', '列表支持按任务名称搜索和按清洗状态筛选。', '列表分页与生产环境一致默认 20 条/页，行操作展示启动、编辑、删除和更多入口。', '清洗任务详情弹窗拆为清洗详情和清洗日志两个页签；清洗详情按基本信息、清洗结果、算子标签和清洗前后对照表展示。', '当前页以用户迭代需求为基线；生命周期任务支持在列表行内编辑名称和描述，详情和日志读取更新后的任务对象。'],
      productionComparison: ['本轮按用户要求直接参考生产环境；列表字段、筛选区、四步横向说明和分页规格对齐生产环境当前页面。'],
      userChanges: ['旧截图已删除并回退影响；数据清洗页改以生产环境当前页面为基线。', '根据本轮批注，创建页数据集选择对齐推理结果集选择弹窗，清洗字段改为按数据集格式回显单选，删除数据集名称、流程摘要、清洗后数据集和流程结果预期卡片，并补充定时执行时间配置。', '根据本轮批注，删除失败重试次数字段，清洗任务详情调整为基本信息与清洗结果结构，并保留清洗日志页签。', '根据本轮全局任务需求，清洗任务列表支持名称和描述行内编辑，保存后详情、日志和资源引用提示同步刷新。'],
    }),
  },
  {
    match: pathname =>
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '任务名称', location: '列表/表单', type: '文本', required: '是', description: '训练任务唯一标识，列表支持按统一生命周期规则行内编辑。' },
        { name: '任务级描述', location: '列表/任务详情', type: '长文本', required: '否', description: '训练任务整体描述；版本级 taskDescription 继续用于版本配置，不与任务级描述混用。' },
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['根据本轮需求，被推理、部署或训练任务引用的模型暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '任务名称', location: '列表/创建页', type: '文本', required: '是', description: '评估任务主标识，列表支持按统一生命周期规则行内编辑。' },
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '服务名称', location: '列表/创建页', type: '文本', required: '是', description: '部署服务名称，列表支持按统一生命周期规则行内编辑。' },
      ...
      interactionNotes: ['大模型部署页已接入统一任务状态流转规则。', '当前页以用户迭代需求为基线；部署任务支持在列表行内编辑名称和描述，详情读取更新后的任务对象。'],
      productionComparison: ['与生产环境大模型部署页对齐。'],
      userChanges: ['根据本轮全局任务需求，大模型部署任务支持名称和描述行内编辑，保存后列表和详情同步刷新。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['根据本轮需求，在线推理服务可作为文档中心 Agent 的对话模型来源，并支持从 Agent 配置页跳转新建后返回。', '根据本轮需求，在线推理服务补齐删除操作；若被推理或标注任务引用则暂不允许删除，任务完成不释放引用，必须删除任务或停止引用链。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['保留用户明确要求：创建数据集补版本字段，默认 V1；数据类型移除音频，并将文本排序在图片前。', '根据本轮需求，被在线标注任务引用的机器学习数据集暂不允许删除；任务完成不释放引用，必须删除任务或停止引用链。', '根据本轮需求，机器学习数据集补充创建人字段，并限制仅创建人本人、项目管理员或租户管理员可删除。', '根据本轮要求重新参考生产环境，机器学习数据管理详情改为整页版本侧栏 + 基本信息 + 数据详情表结构，并支持删除详情表中的单条数据。', '根据本轮需求，机器学习数据管理上传文件大小放开前端限制。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['对齐生产环境时未覆盖用户已明确要求的机器学习 Notebook、部署和数据管理新增能力。', '根据本轮需求，被推理、部署或训练任务引用的模型暂不允许删除；任务完成、失败或终止不释放引用，必须删除相关任务或停止引用链后才可删除。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        { name: '服务名称', location: '列表/创建页', type: '文本', required: '是', description: '部署服务主标识，列表支持按统一生命周期规则行内编辑。' },
        { name: '服务描述', location: '列表/创建页/详情', type: '长文本', required: '否', description: '部署任务级说明，最多 300 字。' },
      ...
      interactionNotes: [
        ...
        '当前页以用户迭代需求为基线；机器学习部署任务支持在列表行内编辑名称和描述，详情、资源引用拦截和本地持久化 store 同步读取更新结果。',
      ],
      ...
      userChanges: [
        ...
        '根据本轮全局任务需求，机器学习部署任务支持名称和描述行内编辑，保存后列表、详情和引用提示同步刷新。',
      ],
      recentChanges: [
        {
          date: '2026-04-20',
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['本轮明确重新参考生产环境机器学习数据标注页。', '在线标注列表的详情入口进入整页标注详情，而不是普通详情弹窗；详情页展示任务基本信息、数据列表、当前数据和标注操作。', '在线标注与多人标注工作台均在操作区显式提供“删除当前数据”，未提交可删，已提交或已完成锁定不可删。', '多人标注从单一列表补齐为任务总览、标注任务、审核任务三个子视图。', '任务总览仅面向项目管理员/平台管理员；普通成员不展示该 Tab。', '标注任务和审核任务按当前用户领取到的任务展示，且不展示创建标注任务按钮。', '标注任务和审核任务列表存在数据时，行操作必须进入对应标注或审核工作台。', '创建多人标注任务采用独立路由 /machine-annotation/create，而不是普通弹窗。', '创建页保留基本信息、数据选择、处理后数据集、选择标注成员、选择审核成员五个区域。', '在线标注创建弹窗需要对齐生产环境创建流程：通过数据类型过滤可选数据集，选择数据集使用输入框加选择按钮，选择后展示数据量、标注类型和处理后数据集。', '在线标注、多人标注及创建页成员表均设置横向滚动容器和表格滚动宽度，窄屏或右侧需求文档展开时不得挤压重叠；任务名称、数据集名称使用省略并可通过横向滚动查看完整列。', '演示数据需要覆盖文本分类、实体识别、图像分类、物体检测等不同标注任务类型。'],
      ...
      userChanges: ['根据用户本轮要求，机器学习多人标注重新阅读生产环境并补充开发。', '根据浏览器批注，任务总览仅给项目管理员，普通成员只看自己领取到的标注任务和审核任务，创建按钮仅保留在管理员任务总览。', '根据本轮批注，标注任务和审核任务列表补齐行操作入口，并进入对应标注或审核工作台。', '根据本轮需求，机器学习在线标注详情改为整页标注页，和多人标注/审核工作台一起支持删除当前单条数据；任务提交或已完成后不允许删除。', '根据本轮反馈，修复机器学习标注列表列重叠和横向不可滑动问题，并补充多类型演示任务数据。', '根据本轮批注，在线标注创建弹窗去掉标注工具字段，改为生产环境式的数据类型、数据选择、数据集选择、数据量和处理后数据集配置。'],
      recentChanges: [
        {
          date: '2026-05-08',
