...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['本轮明确重新参考生产环境机器学习数据标注页。', '在线标注列表的详情入口进入整页标注详情，而不是普通详情弹窗；详情页展示任务基本信息，并根据标注类型分流到不同工作台。', '在线标注详情已拆分文本分类、实体识别、图像分类、物体检测、图像分割五类工作台：文本分类展示文本与单/多标签选择；实体识别展示左侧标签栏、中间实体文本区、右侧实体结果；图像分类展示图片预览与标签面板；物体检测展示矩形框画布和框列表；图像分割展示分割画布和区域列表。', '在线标注与多人标注工作台均在操作区显式提供“删除当前数据”，未提交可删，已提交或已完成锁定不可删。', '多人标注从单一列表补齐为任务总览、标注任务、审核任务三个子视图。', '任务总览仅面向项目管理员/平台管理员；普通成员不展示该 Tab。', '标注任务和审核任务按当前用户领取到的任务展示，且不展示创建标注任务按钮。', '标注任务和审核任务列表存在数据时，行操作必须进入对应标注或审核工作台。', '创建多人标注任务采用独立路由 /machine-annotation/create，而不是普通弹窗。', '创建页保留基本信息、数据选择、处理后数据集、选择标注成员、选择审核成员五个区域。', '在线标注创建弹窗需要对齐生产环境创建流程：通过数据类型过滤可选数据集，选择数据集使用输入框加选择按钮，选择后展示数据量、标注类型和处理后数据集。', '在线标注、多人标注及创建页成员表均设置横向滚动容器和表格滚动宽度，窄屏或右侧需求文档展开时不得挤压重叠；任务名称、数据集名称使用省略并可通过横向滚动查看完整列。', '演示数据需要覆盖文本分类、实体识别、图像分类、物体检测、图像分割等不同标注任务类型。'],
      ...
      userChanges: ['根据用户本轮要求，机器学习多人标注重新阅读生产环境并补充开发。', '根据浏览器批注，任务总览仅给项目管理员，普通成员只看自己领取到的标注任务和审核任务，创建按钮仅保留在管理员任务总览。', '根据本轮批注，标注任务和审核任务列表补齐行操作入口，并进入对应标注或审核工作台。', '根据本轮需求，机器学习在线标注详情改为整页标注页，和多人标注/审核工作台一起支持删除当前单条数据；任务提交或已完成后不允许删除。', '根据本轮计划，在线标注详情按 annotationType 拆为五种可见差异化工作台，并补充图像分割演示任务。', '根据本轮反馈，修复机器学习标注列表列重叠和横向不可滑动问题，并补充多类型演示任务数据。', '根据本轮批注，在线标注创建弹窗去掉标注工具字段，改为生产环境式的数据类型、数据选择、数据集选择、数据量和处理后数据集配置。'],
      recentChanges: [
        {
          date: '2026-05-08',
