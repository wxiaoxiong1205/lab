...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      actions: [
        ...
        { name: '编辑', entry: '列表行操作', precondition: '集群存在', successFeedback: '更新集群名称、描述和 YAML 配置后连接状态重置为未测试', errorFeedback: '表单校验失败时保留弹窗' },
      ...
      interactionNotes: ['当前页已根据第二阶段截图需求补充挂载状态、存储配置、镜像仓库三列，并新增删除按钮。', '集群表格使用横向滚动和明确列宽，避免小屏下右侧字段与操作不可见。', '导入集群弹窗按截图改为文本输入 kubeconfig，并补充集群描述字段；不再展示 API Server、标签、上传按钮和测试连接。', '无论连接成功、失败或未测试，列表均展示编辑按钮；编辑集群信息弹窗按截图改为集群名称、集群描述、集群配置（YAML格式）三段，点击更新后连接状态自动变为未测试。', '列表操作删除三个点详情入口，不再提供集群详情弹窗。'],
      productionComparison: ['本页当前以用户迭代需求为基线，不再以第一阶段生产环境拆解为默认依据。'],
      userChanges: ['根据待迭代项截图，集群管理补充挂载状态、存储配置、镜像仓库字段，并增加删除操作。', '根据批注补充小屏横向滚动，并为连接失败集群开放编辑更新流程。', '根据用户截图重做导入和编辑弹窗，导入页补集群描述并采用文本输入 kubeconfig，编辑页采用 YAML 配置表单。', '根据用户最新要求，编辑按钮不再受连接状态限制，所有集群均可编辑。', '根据用户要求删除操作列三个点详情入口和详情弹窗。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      module: '系统管理',
      status: '已基于需求演进',
      goal: '管理属性配置和标签配置，并把属性、标签和值管理拆成清晰的两层交互。',
      audience: '平台管理员',
      problem: '修正属性和标签配置逻辑，让新增入口可执行，并避免展示不需要的输入方式字段。',
      structure: ['属性配置 Tab', '标签配置 Tab', '属性新增弹窗', '标签列表', '标签值弹窗'],
      fields: [
      ...
      interactionNotes: ['当前页已根据用户需求把“添加标签”和“添加标签值”拆成两层操作。', '添加属性按钮已接入新增属性弹窗，属性列表移除“输入方式”列，仅展示属性名称、属性描述、属性值、属性分组和是否必填。'],
      productionComparison: ['本页当前以第二阶段用户需求为基线。'],
      userChanges: ['根据待迭代项截图，添加标签仅添加标签，新增标签值需要进入“查看标签值”后处理。', '根据本轮批注修复添加属性按钮无响应问题，并移除属性列表的输入方式字段。'],
    }),
  },
  {
    match: pathname => pathname === '/machine-model-deployment' || pathname.startsWith('/machine-model-deployment/'),
