...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['页面头部不再展示面包屑，统一只保留标题、返回按钮和必要操作。', '去掉面包屑后，列表页头部采用克制的标题 + 说明结构，不新增统计卡；创建页、详情页和新增版本页统一改为“返回 + 标题 + 说明/操作”的紧凑结构。', '版本信息需要与列表信息联动展示。', '列表表格统一使用普通横向滚动，不再使用固定操作列，避免内容与操作区重叠。', '详情页采用左侧版本切换、右侧信息与明细展示的结构。', '本页已基于用户新增需求补充 DPO / RFT（PPO、GRPO）对应数据用途，并改为单字段内的二级选择。', '创建测试数据时，DPO 的数据格式展示对齐训练数据集为 CHOSEN_REJECTED，RFT-PPO/RFT-GRPO 自动切回 PROMPT_RESPONSE，避免保留 SFT 场景下的 ROLE_BASED。'],
      productionComparison: ['保持与生产环境测试数据管理一致，不扩展业务测试数据集能力。'],
      userChanges: ['根据用户新增需求，测试数据用途扩展为 SFT、DPO、RFT-PPO、RFT-GRPO 的文本生成/图像理解组合。', '根据用户最新反馈，测试数据创建页的数据格式需要跟随 DPO/PPO/GRPO 数据用途自动联动，其中 DPO 对齐训练数据集显示为 CHOSEN_REJECTED。'],
    }),
  },
  {
    match: pathname =>
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      states: [
        ...
        { name: '已停止', meaning: 'Notebook 未运行', presentation: '状态标签提示停止', availableActions: '启动、查看详情、发布为案例' },
      ],
      interactionNotes: [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['根据本轮需求，在线推理服务可作为文档中心 Agent 的对话模型来源，并支持从 Agent 配置页跳转新建后返回。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['保留用户明确要求：创建数据集补版本字段，默认 V1；数据类型移除音频，并将文本排序在图片前。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      userChanges: ['对齐生产环境时未覆盖用户已明确要求的机器学习 Notebook、部署和数据管理新增能力。'],
    }),
  },
  {
    ...
    doc: createDoc({
      ...
      module: '文档中心',
      status: '已基于需求演进',
      goal: '提供平台使用文档，并在存在运行中 Agent 服务时支持自然语言问答查找文档。',
      audience: '平台用户、平台管理员、算法工程师',
      problem: '让用户不必只依赖左侧目录和关键词搜索，也能通过对话快速定位到对应文档章节。',
      structure: ['文档中心左侧目录', '文档正文', 'Agent助手右侧对话栏', '回答引用与文档定位'],
      fields: [
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', 'Agent助手只在存在运行中服务时展示；回答必须带 citations，不能只返回自然语言。', '引用定位优先跳转文档中心章节，必要时可跳转到系统配置等被引用页面。'],
      productionComparison: ['本页当前不参考生产环境，以第二阶段用户需求为基线。'],
      userChanges: ['根据本轮需求，文档中心接入由系统配置 Agent助手控制的全局 RAG 对话入口。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '模型类型', location: '筛选区/列表/表单', type: '枚举', required: '是', description: '当前仅支持文本生成、图像理解。' },
        { name: '模型提供商', location: '筛选区/列表/表单', type: '枚举', required: '是', description: '当前仅支持 Qwen。' },
      ...
      interactionNotes: ['模型仓库页面已按用户提供的全局任务流转规则收口。', '新增模型弹窗按用户截图调整为“本地 / ModelScope”来源切换：本地来源使用模型 Code 下拉，ModelScope 来源补集群选择、下载入口提示和任务定时配置。', '本地和 ModelScope 两种来源的模型类型与模型提供商保持一致：模型类型为文本生成、图像理解，模型提供商当前仅 Qwen。', '筛选区支持换行，列表设置明确列宽与横向滚动，避免小屏内容错位或右侧操作不可见。'],
      productionComparison: ['以生产环境基础模型管理页为基线，并按当前命名调整为模型仓库。'],
      userChanges: ['根据待迭代项截图，本地上传模型操作中不再展示日志能力，新增模型补模型来源选择，支持本地和 ModelScope。', '根据本轮批注重做新增基础模型弹窗字段顺序和来源差异，并修复模型仓库小屏错位与无法横向滑动问题。', '根据最新批注将模型类型从 LLM/VLM 改为文本生成/图像理解，并将模型提供商收敛为 Qwen。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: '存储类型', location: '新建弹窗/基本信息', type: '下拉选择', required: '是', description: '支持火山引擎 TOS、MinIO、NFS、华为云 OBS。' },
      ...
      interactionNotes: ['当前页已根据第二阶段截图需求补充删除按钮，并增加“已绑定集群，不允许删除”的约束，未绑定集群删除前仍需二次确认。', '搜索筛选区使用可换行布局，表格设置明确列宽与横向滚动，避免小屏下内容错位或右侧操作不可见。', '新建存储配置弹窗按用户截图调整为“基本信息”和“配置参数”两块卡片；存储类型下拉使用火山引擎 TOS、MinIO、NFS、华为云 OBS；配置参数对齐 TOS 场景字段。', '列表 mock 数据和新建数据统一使用新的存储类型；查看详情弹窗同步改为基本信息和配置参数两块卡片，Secret Key 掩码展示。'],
      productionComparison: ['本页当前以用户迭代需求为基线，不再以第一阶段生产环境拆解为默认依据。'],
      userChanges: ['根据待迭代项截图，存储管理操作区新增删除按钮，且在已绑定集群时禁止删除。', '根据用户反馈修复小屏错位和列表无法横向滑动的问题。', '根据用户截图重做新建存储配置弹窗，补齐基本信息、配置参数、TOS 参数字段与创建配置按钮。', '根据用户反馈同步列表侧存储类型、删除二次确认和详情页字段结构。'],
    }),
  },
  {
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      audience: '平台管理员',
      problem: '让平台管理员既能维护基础配置，也能为文档中心配置可运行的全局 RAG 问答服务。',
      structure: ['属性配置 Tab', '标签配置 Tab', 'Agent助手 Tab', 'Agent 文档中心目录', 'Agent 服务列表', 'Agent 服务新建/编辑弹窗', '标签值弹窗'],
      fields: [
        ...
        { name: '对话模型配置', location: 'Agent助手弹窗', type: '在线推理服务或自定义 API', required: '是', description: '用于生成文档问答结果；高级参数包含 max_tokens、Temperature、Top_p、presence_penalty。' },
      ],
      actions: [
        ...
        { name: '新建 Agent 服务', entry: 'Agent助手 Tab 右上角按钮', precondition: '进入文档中心目录', successFeedback: '新增一个未启动的 Agent 服务', errorFeedback: '表单校验失败或接口不可用' },
        { name: '启动 Agent 服务', entry: 'Agent 服务行操作', precondition: '服务配置完整', successFeedback: '当前服务启动，其他运行中服务自动停止', errorFeedback: '索引或模型配置不可用时提示失败' },
      ...
      states: [
        ...
        { name: 'Agent 未启动', meaning: '已有服务但未对文档中心生效', presentation: '服务状态为未启动，文档中心不展示助手', availableActions: '编辑、启动、测试、重建索引、删除' },
        { name: 'Agent 运行中', meaning: '该服务是全局唯一生效的文档中心助手', presentation: '绿色运行中状态，文档中心展示 Agent 助手', availableActions: '停止、编辑、测试、重建索引' },
      ],
      interactionNotes: ['当前页已根据用户需求把“添加标签”和“添加标签值”拆成两层操作。', '添加属性按钮已接入新增属性弹窗，属性列表移除“输入方式”列，仅展示属性名称、属性描述、属性值、属性分组和是否必填。', 'Agent助手当前以用户迭代需求为基线，不参考生产环境；服务作用域固定为全局文档中心。', '对话模型可选择在线推理服务，也可自行配置 API；如果没有可用在线推理服务，可跳转在线推理服务创建页。', '保存时如果 Embedding 的 API地址、API Key 或模型名称发生变化，必须弹窗提示“修改向量模型会触发知识库向量重新构建，是否重新构建？”并提供是/否。'],
      productionComparison: ['本页当前以第二阶段用户需求为基线。'],
      userChanges: ['根据待迭代项截图，添加标签仅添加标签，新增标签值需要进入“查看标签值”后处理。', '根据本轮批注修复添加属性按钮无响应问题，并移除属性列表的输入方式字段。', '根据本轮需求新增 Agent助手 Tab：在文档中心目录下配置全局 Agent 服务，拆分 Embedding、Rerank 和对话模型配置，并接入最小 RAG 接口契约。'],
    }),
  },
  {
    match: pathname => pathname === '/machine-model-deployment' || pathname.startsWith('/machine-model-deployment/'),
