  ...
  },
  {
    ...
    doc: createDoc({
      ...
      fields: [
        ...
        { name: 'GRPO训练参数配置', location: '创建表单/训练配置', type: '选择框 + 可编辑模板', required: 'GRPO时是', description: '用于选择一套 GRPO 训练参数模板，选择后直接展示整段模板内容，用户可在本次任务内继续编辑。' },
...
GRPO 创建页不再让用户从固定默认参数开始填写，而是通过系统配置中的 GRPO 训练参数配置快速带出微调类型和训练参数。
...
- 当训练方法选择 RFT 且算法选择 GRPO 时，展示 GRPO训练参数配置字段。
...
- 模型仓库中维护的模型可被大模型训练创建页读取并展示。`,
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      structure: ['属性配置 Tab', '标签配置 Tab', 'GRPO训练参数配置 Tab', '标签配置左侧分组', 'Agent助手 Tab', '文档中心单目录', 'Agent服务单配置卡', 'Agent 服务配置弹窗', '标签值弹窗'],
      fields: [
        ...
        { name: 'GRPO训练参数配置', location: 'GRPO训练参数配置 Tab', type: '参数配置列表 + 代码模板弹窗', required: '否', description: '把 GRPO 常用微调类型和训练参数沉淀为可复用配置；新增或编辑时只填写模板名称、描述，并直接维护一整段参数模板内容。' },
      ...
      actions: [
        ...
        { name: '维护GRPO训练参数配置', entry: 'GRPO训练参数配置 Tab', precondition: '进入系统配置', successFeedback: '配置保存后可在 GRPO 创建页选择', errorFeedback: '模板字段越界或基础信息校验失败时提示' },
...
- 参数模板内容采用 JSON 结构作为 YAML 风格模板的当前实现，根字段只允许 fineTuneType 和 params，params 只允许当前产品已有训练参数字段。
- 系统内置默认全参配置和默认 LoRA 配置，保证 GRPO 创建页有可选配置。

## 4. 验收标准

- 系统配置页可进入 GRPO训练参数配置 Tab。
- 新增或编辑配置后，可在大模型训练创建页 RFT-GRPO 场景选择。
- 停用配置不出现在创建页可选列表。
- JSON 输入奖励规则、DeepSpeed、模型、数据或资源字段时需要校验失败。`,
        },
      ],
    }),
  },
