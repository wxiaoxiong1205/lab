...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      actions: [
        ...
        { name: '编辑 Notebook', entry: '已停止/已终止/失败 Notebook 的列表行操作或详情页顶部操作', precondition: 'Notebook 未处于运行中、启动中或排队中', successFeedback: '进入编辑页回填原创建配置；保存后更新配置并返回详情页，重新启动后生效', errorFeedback: '运行中进入编辑页时提示需先停止' },
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['当前页以用户迭代需求为基线。', '旧结构：基础模型创建页按固定 Qwen 系列筛选模型版本并直铺在创建页；新结构：基础模型选择改为弹窗，弹窗左侧先选模型提供商，右侧展示该提供商下匹配当前训练类型的模型。', '本期以 Qwen 作为已适配模型示例，已下载模型可选，未下载模型展示“未下载，请联系管理员”；模型仓库新增的其它提供商模型也允许选择，但标记为“未适配”。'],
      productionComparison: ['本轮未参考生产环境，按用户新增需求在当前仓库基线上迭代。'],
      userChanges: ['根据本轮需求，大模型训练创建页的基础模型选择改为展示模型仓库所有匹配训练类型的模型；Qwen 作为本期已适配示例，已下载可选，未下载提示联系管理员；其它提供商模型也可选择并标记未适配。', '根据本轮调整，基础模型选择改成弹窗：先选模型提供商，再选具体模型，避免在创建页直铺大量模型。'],
    }),
  },
  {
