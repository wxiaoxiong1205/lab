...
const pageDocs: PageDocEntry[] = [
  ...
  },
  {
    ...
    doc: createDoc({
      ...
      interactionNotes: ['页面头部不再展示面包屑，统一只保留标题、返回按钮和必要操作。', '去掉面包屑后，列表页头部采用克制的标题 + 说明结构，不新增统计卡；创建页、详情页和新增版本页统一改为“返回 + 标题 + 说明/操作”的紧凑结构。', '版本信息需要与列表信息联动展示。', '列表表格统一使用普通横向滚动，不再使用固定操作列，避免内容与操作区重叠。', '列表页只允许编辑测试数据集名称，不新增描述列；描述属于版本维度，只在详情页当前版本基本信息中编辑。', '字段进入编辑态后，取消按钮在前、确认按钮在后；详情页名称和描述字段后常显编辑按钮。', '详情页采用左侧版本切换、右侧信息与明细展示的结构。', '测试数据创建和新增版本的文件上传需展示进度，上传中可取消，失败或取消后可继续上传并保留断点进度；文件大小不设前端限制。', '根据用户最新要求，测试数据管理的数据用途从“文本生成/图像理解 + SFT/DPO/RFT 二级选择”收敛为单级枚举；创建页、列表筛选、列表列、详情页和新增版本页均只展示文本生成/图像理解。', '历史 mock 数据中带 SFT、DPO、RFT 前缀的数据用途在测试数据页按文本生成/图像理解归一展示。'],
