  ...
  const handleSubmitPort = async () => {
    ...
    try {
      ...
      const nextPort: OpenPortRecord = {
        id: editingPortId ?? `ml-port-${Date.now()}`,
              ...
              <Card size="small" style={sectionCardStyle}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <Form.Item label="数据集" name="dataset">
                    <Select allowClear placeholder="请选择数据集" options={mlDatasetOptions} />
                  </Form.Item>
                  <Form.Item label="大模型" name="model">
                    <Select allowClear placeholder="请选择模型" options={mlModelOptions} />
                  </Form.Item>
                </div>
              </Card>
